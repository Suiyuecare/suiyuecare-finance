-- Permit an applicant who is also the explicitly frozen final accountant to
-- post the voucher after every earlier approval has completed. This is not a
-- general self-approval exception: the request must already be pending a
-- voucher, the active step must be the last accounting step, and its frozen
-- UID/email must name the current actor. finalize_expense_request continues
-- to enforce the completed prior route, exact one-step transition, owned
-- attachments, balanced entries, and immutable posting audit.
--
...
begin
  ...
  where proc_row.oid = v_oid
    ...
    and not pg_catalog.has_function_privilege(
      'public', proc_row.oid, 'EXECUTE'
    )
    and not pg_catalog.has_function_privilege(
      'anon', proc_row.oid, 'EXECUTE'
    )
    and not pg_catalog.has_function_privilege(
      'authenticated', proc_row.oid, 'EXECUTE'
    )
    and not pg_catalog.has_function_privilege(
      'service_role', proc_row.oid, 'EXECUTE'
    );
  ...
  if pg_catalog.strpos(
       v_actual_source,
       'Final voucher posting is the only exception'
     ) = 0
    ...
    raise exception
      'finance_expense_actor_can_act did not install the narrow final-post exception';
  end if;
end;
$migration$;
...
comment on function private.finance_expense_actor_can_act(
  ...
  'Validates the frozen expense step actor. Self-approval stays forbidden except for an explicitly frozen applicant performing the final pending-voucher accounting post.';
