-- Search formatting only. Original identity, participant, tenant/environment,
-- grouping, paging and RPC privileges are preserved. Release owns transaction.
set local lock_timeout='5s';
set local statement_timeout='60s';

create function private.finance_history_search_text_v1(p_value text) returns text
...
create function private.finance_history_amount_text_v1(p_value text) returns text
language plpgsql immutable set search_path='' as $search$
declare s text:=private.finance_history_search_text_v1(p_value);sign text;whole text;fraction text;
begin
 if s!~'^[+-]?\d+(?:\.\d+)?$' then return null;end if;
 sign:=case when left(s,1)='-' then '-' else '' end;
 s:=regexp_replace(s,'^[+-]','');whole:=regexp_replace(split_part(s,'.',1),'^0+(?=\d)','');fraction:=rtrim(split_part(s,'.',2),'0');
 if whole='0' and fraction='' then sign:='';end if;
 return sign||whole||case when fraction<>'' then '.'||fraction else '' end;
end;
$search$;

create function private.finance_history_document_search_v1(p_query text,p_text text,p_amounts jsonb) returns boolean
language plpgsql immutable set search_path='' as $search$
declare word text;term text;candidate text;haystack text:=private.finance_history_search_text_v1(p_text);amounts text[];matched boolean;
begin
 select array_agg(private.finance_history_amount_text_v1(value#>>'{}')) into amounts
...
alter function private.finance_history_search_text_v1(text) owner to postgres;
alter function private.finance_history_amount_text_v1(text) owner to postgres;
alter function private.finance_history_document_search_v1(text,text,jsonb) owner to postgres;
revoke all on function private.finance_history_search_text_v1(text),private.finance_history_amount_text_v1(text),private.finance_history_document_search_v1(text,text,jsonb) from public,anon,authenticated,service_role;
...
begin
  ...
  grouped as (
    select
      ...
      lower(string_agg((source.source_row-array['amount','total','tax','estimated_amount','actual_amount','bank_fee_amount'])::text, ' ' order by source.source_no, source.source_id)) as searchable_text,
  ...
  filtered as (
    select *
    from grouped
    where private.finance_history_document_search_v1(v_search,coalesce(record_no,'')||' '||searchable_text,
...
alter function public.finance_approval_participant_history_for_current_user(integer,integer,text,text) owner to postgres;
revoke all on function public.finance_approval_participant_history_for_current_user(integer,integer,text,text) from public,anon,authenticated,service_role;
grant execute on function public.finance_approval_participant_history_for_current_user(integer,integer,text,text) to authenticated,service_role;
