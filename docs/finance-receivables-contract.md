...
- Nonzero unlinked or ambiguous journal rows retain separate gross debit and credit amounts. For separate debit 70 and credit 70 rows, `unmappedLedgerNet=0`, `unmappedDebitAmount=70`, `unmappedCreditAmount=70`, `unmappedEntryCount=2`, and `needsReview=true`. If both amounts occur on the same row, the count is 1 and the same warning remains; a row with both sides zero does not count. `scopeDifference = ledgerNet - mappedLedgerNet - unmappedLedgerNet` additionally flags mismatched invoice/ledger dimensions. No original invoice or journal is changed.
