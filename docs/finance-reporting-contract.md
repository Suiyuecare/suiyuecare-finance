...
### Source integrity and autonomous book-tax reads (2026-09-11)

- New or changed classifications must carry a server-issued SHA-256 `sourceBinding`. The existing profile CAS/audit transaction locks each changed source and compares its current fingerprint before saving. Source version, certificate detail or evidence changes reject the stale review with `40001`; the client cannot attest its own source.
- Existing classifications are retained without automatically adding a binding. Old audited revisions remain immutable. Unbound or changed sources require explicit re-review and cannot contribute usable filing totals. Original certificate VAT remains independent of book VAT; utility gross-expense policy is unchanged.
- Workpapers fetch source verification in batches and independently load verified posting details for the selected company and filing period, including unknown dates and changed cross-period classifications. Source reads and posting reads honor session, permission, object/version and request-generation boundaries. Unrelated prior-period advances do not block current book reads.
- Loading or failed remote reads block export. A verified voucher with no unique certificate-line match produces `BOOK_TAX_UNRESOLVED`, never a fabricated zero or a ready-for-review workpaper. This incomplete workpaper can be exported with its explicit reconciliation checks. Unposted and explicitly out-of-scope sources do not borrow tax from draft accounting rows.
- Export re-verifies source versions. PDF clicks reserve a blank window synchronously; report content is written only after validation. Failure or an identity/company/period change closes the reserved window without writing the report.
- Regression commands: `node scripts/check_tax_source_integrity.cjs` (real PostgreSQL migration/RPC, ACL, immutable history, stale source rejection and exact rollback canaries) and `node scripts/check_tax_source_runtime.cjs` (actual workspace/engine functions, async races, book-tax exports and PDF activation).
