...
try{
 ...
 await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');assert.equal(await fp(),applied);writeLedger([...prerequisites,...guard.HISTORY_PERFORMANCE_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/read latency migration batch/);writeLedger([...prerequisites,...guard.HISTORY_PERFORMANCE_MIGRATIONS,...guard.READ_LATENCY_MIGRATIONS]);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/AR mapping migration batch/);assert.throws(()=>apply('reapply'),/pending/);check();
 // Later read-only RPC contract is an explicit no-op fixture here; its own suite executes the actual migration.
 write('finance_statement_source_postflight.sql','\\set ON_ERROR_STOP on\nselect 1;\n');
 for(const p of [phase]){const out=path.join(dir,p+'_recovery.sql');guard.preparePhaseQuery(path.join(dir,postflights[0]),out,p,p===phase?versions:'none');const raw=fs.readFileSync(out,'utf8');assert.match(raw,/^begin read only;/);for(const name of postflights)assert.ok(raw.includes('-- Reviewed reports postflight: '+name));await db.exec(raw);assert.equal(await fp(),applied);check();}
...
`);
  ...
  fs.writeFileSync(path.join(realDir,'finance_statement_source_postflight.sql'),'\\set ON_ERROR_STOP on\nselect 1;\n');
  for(const current of [phase]){const file=path.join(realDir,current+'-readonly.sql');guard.preparePhaseQuery(path.join(realDir,postflights[0]),file,current,current===phase?versions:'none');await real.exec(fs.readFileSync(file,'utf8'));assert.equal(await realFp(),realApplied);check();}
