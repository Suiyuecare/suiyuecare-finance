...
for(const name of [...guard.INVOICE_READ_SCOPE_POSTFLIGHT_FILES,...guard.AR_READ_SCOPE_POSTFLIGHT_FILES,...guard.HR_BRIDGE_POSTFLIGHT_FILES,...guard.AUDIT_SECURITY_POSTFLIGHT_FILES])write(name,'\\set ON_ERROR_STOP on\nselect 1;\n');
...
try{
 ...
 writeLedger([...prerequisites,...batch,...guard.INVOICE_READ_SCOPE_MIGRATIONS]);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/AR read scope migration batch/);writeLedger([...prerequisites,...batch,...guard.INVOICE_READ_SCOPE_MIGRATIONS,...guard.HR_BRIDGE_MIGRATIONS]);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/audit security migration batch/);writeLedger([...prerequisites,...batch,...guard.INVOICE_READ_SCOPE_MIGRATIONS,...guard.HR_BRIDGE_MIGRATIONS,...guard.AUDIT_SECURITY_MIGRATIONS,...guard.HR_DIRECTORY_EXPORT_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');check();
 for(const p of [phase,'frontend_compat']){const file=output('recovery');guard.preparePhaseQuery(path.join(dir,posts[0]),file,p,p===phase?versions:'none');const raw=fs.readFileSync(file,'utf8');assert.match(raw,/^begin read only;/);assert.equal((raw.match(/-- Reviewed reports postflight:/g)||[]).length,p===phase?19:24);assert.equal(raw.includes('finance_invoice_select_initplan_postflight.sql'),p==='frontend_compat');assert.doesNotMatch(raw,/insert into supabase_migrations|create or replace function/);await db.exec(raw);assert.equal(await fp(),applied);check();}
