...
(async()=>{
 ...
 await browser('open','https://127.0.0.1:'+app.address().port+'/');await browser('eval',"document.getElementById('download').click()");await browser('wait','800');
 const saved=fs.readdirSync(downloads);assert.deepEqual(saved,['PUR-001_採購照護耗材 & 清潔用品.xlsx']);
 assert.equal(crypto.createHash('sha256').update(fs.readFileSync(path.join(downloads,saved[0]))).digest('hex'),crypto.createHash('sha256').update(content).digest('hex'),'Downloaded bytes equal the original XLSX; naming never changes file contents');
 ...
 fs.writeFileSync(path.join(out,'result.json'),JSON.stringify({ok:true,filename:saved[0],bytes:content.length,requests},null,2));
