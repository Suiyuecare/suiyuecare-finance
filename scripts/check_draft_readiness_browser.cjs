...
async function until(condition){return scope('(async function(){for(var i=0;i<150;i++){if('+condition+')return true;await new Promise(r=>setTimeout(r,20));}return false;})()');}
...
window.__mode=mode;window.__reads=[];window.__writes=[];window.__late=[];window.__aborts=[];window.__alerts=[];window.__broadDone=false;window.__started=performance.now();
var nativeTimer=window.setTimeout;window.setTimeout=function(fn,ms){return nativeTimer(fn,ms===12000?500:ms===15000?700:ms===20000?1000:ms===120000?1600:ms)};
...
function transport(name,args,payload){
 ...
 return{abortSignal:function(signal){signal.addEventListener('abort',function(){window.__aborts.push(name)});return this;},then:function(resolve,reject){
   ...
   else if(requestedMode==='source-hang'&&['expense_requests','bills','invoices'].indexOf(name)>-1)promise=new Promise(function(){});
...
getSb=function(){return{
 from:function(table){
   ...
   ['insert','upsert','update','delete'].forEach(function(method){q[method]=function(){window.__writes.push({table:table,method:method});throw Error('Offline read-only fixture refuses '+method)}});
   return q;
 },
 rpc:function(name,args){return transport(name,args||{},{error:{code:'42501',message:'Offline fixture only authorizes direct draft and bootstrap reads'}})},
...
async function noErrors(label){check(label+' has no unhandled browser errors',(await b('errors')).trim()==='');check(label+' made no mutations',await scope('window.__writes.length===0'));}
...
check('drafts become ready while broad source reads are still pending',await until(ready()+'&&!window.__broadDone&&window.__reads.some(x=>["expense_requests","bills","invoices"].includes(x.name)&&!x.ended)'));
...
check('broad timeout cannot revoke independently ready drafts',await until('window.__broadDone&&'+ready()));
...
console.log('PASS draft readiness browser: '+out);
 ...
 try{debug=await scope('({mode:window.__mode,reads:window.__reads,writes:window.__writes,alerts:window.__alerts,state:typeof draftReadinessForCurrentUser==="function"?draftReadinessForCurrentUser():null,text:document.body.innerText,files:S.nrFiles})');}catch(_){}
