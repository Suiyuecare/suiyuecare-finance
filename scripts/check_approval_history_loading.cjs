...
(async()=>{
 for(const mode of ['57014','hang','sync']){
  ...
  c.getSb=()=>({rpc:()=>Promise.resolve({data:payload(c)})});assert((await c.window.loadApprovalHistoryPage({force:true})).ok);check(mode+' retry restores verified summaries/count without mutating source cache',c.APPROVAL_HISTORY_RUNTIME.total===1&&c.APPROVAL_HISTORY_RUNTIME.items.length===1&&c.REQS.length===0);
 }
 {
  const c=fixture((args,c)=>Promise.resolve({data:payload(c,{offset:args.p_offset,total:123})}));let ids=[];
  for(let p=1;p<=3;p++){const out=await c.loadApprovalHistoryPage({page:p});assert(out.ok);ids.push(...out.items.map(x=>x.recordId));assert.equal(c.APPROVAL_HISTORY_RUNTIME.total,123);assert.equal(c.S.apprPage,p);}
 ...
 }
 {
  ...
  const old=c.loadApprovalHistoryPage({query:'old'});await tick();const oldPayload=payload(c,{ids:['OLD']});await c.loadApprovalHistoryPage({force:true,query:'new'});resolveOld({data:oldPayload});assert((await old).stale);check('slow old search cannot overwrite newer results or cache',c.REQS.length===0&&c.APPROVAL_HISTORY_RUNTIME.items[0].recordId==='NEW');
 }
 ...
 for(const bad of ['total-null','total-string','all-missing','rows-truncated','wrong-page','duplicate','wrong-auth','wrong-env','empty-summary']){
  const c=fixture((args,c)=>{const p=payload(c,{total:2});if(bad==='total-null')p.total=null;if(bad==='total-string')p.total='2';if(bad==='all-missing')delete p.all_total;if(bad==='rows-truncated')p.items.pop();if(bad==='wrong-page')p.page.offset=50;if(bad==='duplicate')p.items[1]=p.items[0];if(bad==='wrong-auth')p.identity.auth_user_id='OTHER';if(bad==='wrong-env')p.identity.data_environment='production';if(bad==='empty-summary')p.items[0].summary.source_count=0;return Promise.resolve({data:p});});
 ...
 {
  const c=fixture((args,c)=>{const p=payload(c,{total:2});p.items[1].summary.amount='malformed';return Promise.resolve({data:p});});const out=await c.loadApprovalHistoryPage();check('a malformed late summary cannot partly apply an incomplete page',!out.ok&&c.REQS.length===0&&c.APPROVAL_HISTORY_RUNTIME.items.length===0);
 }
