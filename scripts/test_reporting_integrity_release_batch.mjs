...
const dir=fs.mkdtempSync(path.join(os.tmpdir(),'finance-reporting-integrity-release-'));
const migrationDir=path.join(dir,'migrations');fs.mkdirSync(migrationDir);
const phase=guard.RELEASE_PHASE_DATABASE_REPORTING_INTEGRITY,versions=guard.REPORTING_INTEGRITY_MIGRATIONS.join(',');
const prerequisites=['20260820000000',...guard.MIGRATION_CHAIN,...guard.REVIEWED_POST_BASELINE_MIGRATIONS,guard.MIGRATION_HUMAN_ACCOUNTING_AUTHORITY,...guard.AUDIT_MIGRATIONS,...guard.CASE_MIGRATIONS,...guard.UTILITY_MIGRATIONS,...guard.REPORT_MIGRATIONS,...guard.AMOUNT_SEARCH_MIGRATIONS];
...
const secondSource='create function public.reporting_integrity_second_probe() returns boolean language sql as $$select true$$;';
for(const version of guard.REVIEWED_MIGRATION_CATALOG)fs.writeFileSync(path.join(migrationDir,version+'_fixture.sql'),version===guard.REPORTING_INTEGRITY_MIGRATIONS[0]?source:version===guard.REPORTING_INTEGRITY_MIGRATIONS[1]?secondSource:'select 1;');
const postflights=guard.REPORT_POSTFLIGHT_FILES.concat('finance_amount_search_postflight.sql',guard.REPORTING_INTEGRITY_POSTFLIGHT_FILES);
const postSources=postflights.map((name,i)=>`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":''}do $post_${i}$ begin if to_regprocedure('public.amount_search_release_probe()') is null or to_regprocedure('public.reporting_integrity_second_probe()') is null or (select id from public.application_accounting_lines)<>2 then raise exception 'postflight must run after migration';end if;end;$post_${i}$;\n`);
...
const fingerprint=path.join(repo,'scripts/finance_reporting_integrity_fingerprint.sql'),fpSql=fs.readFileSync(fingerprint,'utf8').replace(/^\\set ON_ERROR_STOP on\n/,'');
...
const canaries=['auth','finalize','utility','ar','profiles','amount','reconcile','taxsource'].map((name,i)=>canary(name,10+i));
const rehearsal=(name,paths=canaries)=>{const output=path.join(dir,name+'.sql');guard.prepareAuditBatchRehearsal(migrationDir,output,versions,fingerprint,paths[0],arPost,phase,paths[1],paths[2],paths.slice(3,5),profilePost,paths[5],paths.slice(6,8));return fs.readFileSync(output,'utf8');};
...
assert.equal(versions,'20260911135457,20260911135514');
for(const installed of guard.REPORTING_INTEGRITY_MIGRATIONS){writeLedger([...prerequisites,installed]);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),/partially installed/);check();}writeLedger(prerequisites);
assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'pending');
assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/reporting integrity migration batch/);
...
for(const value of ['none',guard.REPORTING_INTEGRITY_MIGRATIONS.slice().reverse().join(','),guard.REPORT_MIGRATIONS.join(','),versions+',20260911135515']){assert.throws(()=>guard.releasePlan(phase,value));check();}
writeLedger([...prerequisites,'20260911135515']);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),/unreviewed/);writeLedger(prerequisites);check();
for(let slot=0;slot<8;slot++){const paths=canaries.slice();paths[slot]=null;assert.throws(()=>rehearsal('missing_canary_'+slot,paths));check();}
const db=new PGlite();
try{
 ...
 for(const name of ['auth','finalize','utility','ar','profiles','amount','reconcile','taxsource']){assert.ok(sql.indexOf('$'+name+'_core$')<sql.indexOf('rollback to savepoint'));assert.ok(sql.indexOf('$'+name+'_rollback$')>sql.indexOf('rollback to savepoint'));}
 await db.exec(sql);assert.equal(await fp(),original);check();
 for(let slot=0;slot<8;slot++)for(const failure of ['core','rollback']){const paths=canaries.slice();paths[slot]=canary('failure_'+slot+'_'+failure,30+slot,failure);await assert.rejects(()=>db.exec(rehearsal('rendered_failure_'+slot+'_'+failure,paths)),/intentional/);await db.exec('rollback');assert.equal(await fp(),original);check();}
 for(let i=0;i<postflights.length;i++){
  ...
  for(const mode of ['rehearsal','apply']){await assert.rejects(()=>db.exec(mode==='rehearsal'?rehearsal('bad_post_rehearsal_'+i):apply('bad_post_apply_'+i)),/postflight rejected/);await db.exec('rollback');assert.equal(await fp(),original);assert.equal((await db.query('select count(*)::int as count from supabase_migrations.schema_migrations where version=$1',[guard.REPORTING_INTEGRITY_MIGRATIONS[0]])).rows[0].count,0);check();}
 ...
 const secondPath=path.join(migrationDir,guard.REPORTING_INTEGRITY_MIGRATIONS[1]+'_fixture.sql');fs.writeFileSync(secondPath,secondSource+"do $$begin raise exception 'second migration rejected';end$$;");for(const mode of ['rehearsal','apply']){await assert.rejects(()=>db.exec(mode==='rehearsal'?rehearsal('second_failure_rehearsal'):apply('second_failure_apply')),/second migration rejected/);await db.exec('rollback');assert.equal(await fp(),original);check();}fs.writeFileSync(secondPath,secondSource);
 const atomic=apply('apply');assert.match(atomic,/pg_advisory_xact_lock[\s\S]+lock table supabase_migrations\.schema_migrations/);assert.equal((atomic.match(/^commit;$/gm)||[]).length,1);for(const name of postflights)assert.ok(atomic.indexOf('-- Reviewed reports postflight: '+name)>atomic.indexOf("'20260911135514',array["));
 await db.query('insert into supabase_migrations.schema_migrations(version) values($1)',['20260911135515']);await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');await db.query('delete from supabase_migrations.schema_migrations where version=$1',['20260911135515']);assert.equal(await fp(),original);check();
 await db.exec(atomic);assert.deepEqual((await db.query('select statements from supabase_migrations.schema_migrations where version=$1',[guard.REPORTING_INTEGRITY_MIGRATIONS[0]])).rows[0].statements,[source]);const applied=await fp();
 await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');assert.equal(await fp(),applied);writeLedger([...prerequisites,...guard.REPORTING_INTEGRITY_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/audit readiness migration batch/);assert.throws(()=>apply('reapply'),/pending/);check();
 for(const p of [phase] /* Current frontend is covered by test_audit_readiness_release_batch. */){const out=path.join(dir,p+'_recovery.sql');guard.preparePhaseQuery(path.join(dir,postflights[0]),out,p,p===phase?versions:'none');const raw=fs.readFileSync(out,'utf8');assert.match(raw,/^begin read only;/);for(const name of postflights)assert.ok(raw.includes('-- Reviewed reports postflight: '+name));await db.exec(raw);assert.equal(await fp(),applied);check();}
 for(const [domain,markerKey,result] of [['ar_reconciliation','ar_reconciliation_canary_result',{canary:'authenticated_ar_reconciliation_v1',ok:true,rolled_back:true,scope_preserved:true}],['tax_source_integrity','tax_source_integrity_canary_result',{canary:'authenticated_tax_source_integrity_v1',ok:true,rolled_back:true,source_binding_preserved:true}]]){
 ...
 const actual=path.join(dir,'actual.sql'),args=['prepare-reporting-integrity-rehearsal','--migration-dir',path.join(repo,'supabase/migrations'),'--output',actual,'--migration-versions',versions,'--fingerprint',fingerprint,'--receivables-postflight',path.join(repo,'scripts',postflights[5]),'--profiles-postflight',path.join(repo,'scripts',postflights[6])];
 ['authenticated-canary','case-canary','utility-canary','receivables-canary','profiles-canary','amount-search-canary','ar-reconciliation-canary','tax-source-canary'].forEach((key,i)=>args.push('--'+key,path.join(repo,'scripts',actualCanaries[i])));
 ...
 for(const version of guard.REPORTING_INTEGRITY_MIGRATIONS){const migration=guard.migrationFiles(path.join(repo,'supabase/migrations')).find(name=>name.startsWith(version+'_'));assert.ok(rendered.includes(fs.readFileSync(path.join(repo,'supabase/migrations',migration),'utf8').trimEnd()));}
 ...
 console.log('PASS reporting integrity release: '+checks+' exact phase/prerequisites, eight canaries, ten pre-COMMIT contracts, full rollback fingerprint and strict CLI checks');
