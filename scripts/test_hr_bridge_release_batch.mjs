...
try{
 ...
 writeLedger([...prerequisites,...batch]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/audit security migration batch/);writeLedger([...prerequisites,...batch,...guard.AUDIT_SECURITY_MIGRATIONS,...guard.HR_DIRECTORY_EXPORT_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');assert.throws(()=>rehearsal(),/pending/);assert.throws(()=>apply(),/pending/);check();
 guard.AUDIT_SECURITY_POSTFLIGHT_FILES.forEach(name=>write(name,'\\set ON_ERROR_STOP on\nselect 1;\n'));
 for(const p of [phase,'frontend_compat']){const file=output('recovery');guard.preparePhaseQuery(path.join(dir,posts[0]),file,p,p===phase?versions:'none');const raw=fs.readFileSync(file,'utf8');assert.match(raw,/^begin read only;/);assert.equal((raw.match(/-- Reviewed reports postflight:/g)||[]).length,p===phase?23:24);assert.doesNotMatch(raw,/insert into supabase_migrations|create or replace function/);await db.exec(raw);assert.equal(await fp(),applied);check();}
