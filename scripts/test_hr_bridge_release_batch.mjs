...
`;
const sources=[source,'alter table private.hr_bridge_release_projection add column voucher_posting integer not null default 2;\n'];
...
assert.equal(versions,'20260922072109,20260922075604');check();
...
for(const subset of batch.map(version=>[version])){writeLedger([...prerequisites,...subset]);assert.throws(()=>rehearsal(),/partial atomic migration batch/);assert.throws(()=>apply(),/partial atomic migration batch/);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/complete HR bridge/);check();}
writeLedger(prerequisites);
assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/HR bridge migration batch/);check();
