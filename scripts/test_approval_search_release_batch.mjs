...
for(const suffix of [[],batch.slice(0,2),batch]){writeLedger([...prerequisites,...suffix]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),suffix.length===3?'applied':'pending');if(suffix.length<3){assert.deepEqual(guard.pendingApprovalSearchBatch(migrationDir,versions,ledger,baseline).map(x=>x.version),batch.slice(suffix.length));assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline));}else{assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');assert.throws(()=>rehearsal(),/pending/);assert.throws(()=>apply(),/pending/);}check();}
...
try{
 ...
 try{
  ...
  for(const p of [phase,'frontend_compat']){const file=path.join(realDir,p+'.sql');guard.preparePhaseQuery(path.join(realDir,posts[0]),file,p,p===phase?versions:'none');await real.exec(fs.readFileSync(file,'utf8'));assert.equal(await realFp(),realApplied);check();}
