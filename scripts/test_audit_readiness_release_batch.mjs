...
const source='create function public.audit_readiness_release_probe() returns boolean language sql as $$select true$$;\nupdate public.application_accounting_lines set id=2;';
for(const version of guard.REVIEWED_MIGRATION_CATALOG)fs.writeFileSync(path.join(migrationDir,version+'_fixture.sql'),version===guard.AUDIT_READINESS_MIGRATIONS[0]?source:'select 1;');
const postflights=guard.REPORT_POSTFLIGHT_FILES.concat('finance_amount_search_postflight.sql',guard.REPORTING_INTEGRITY_POSTFLIGHT_FILES,guard.AUDIT_READINESS_POSTFLIGHT_FILES);
const postSources=postflights.map((name,i)=>`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":''}do $post_${i}$ begin if to_regprocedure('public.audit_readiness_release_probe()') is null or (select id from public.application_accounting_lines)<>2 then raise exception 'postflight must run after migration';end if;end;$post_${i}$;\n`);
...
const fingerprint=path.join(repo,'scripts/finance_audit_readiness_fingerprint.sql'),fpSql=fs.readFileSync(fingerprint,'utf8').replace(/^\\set ON_ERROR_STOP on\n/,'');
...
do $${name}_core$ begin
 ...
 if current_user<>'authenticated' or not public.audit_readiness_release_probe() then raise exception 'wrong role or missing migration';end if;
...
do $${name}_rollback$ begin
 if exists(select 1 from public.approval_step_actor_snapshots where id<>1) or exists(select 1 from public.invoices) or to_regprocedure('public.audit_readiness_release_probe()') is not null then raise exception 'canary rollback residue';end if;
 ${failure==='rollback'?"raise exception 'intentional canary rollback failure';":''}
end;$${name}_rollback$;
-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_END
`);
}
const canaries=['auth','finalize','utility','ar','profiles','amount','reconcile','taxsource','readiness'].map((name,i)=>canary(name,10+i));
const rehearsal=(name,paths=canaries)=>{const output=path.join(dir,name+'.sql');guard.prepareAuditBatchRehearsal(migrationDir,output,versions,fingerprint,paths[0],arPost,phase,paths[1],paths[2],paths.slice(3,5),profilePost,paths[5],paths.slice(6,8),paths[8]);return fs.readFileSync(output,'utf8');};
...
assert.equal(versions,'20260911151054');
...
for(const value of ['none',guard.REPORTING_INTEGRITY_MIGRATIONS.join(','),guard.REPORT_MIGRATIONS.join(','),versions+',20260911151055']){assert.throws(()=>guard.releasePlan(phase,value));check();}
writeLedger([...prerequisites,'20260911151055']);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),/unreviewed/);writeLedger(prerequisites);check();
const migrationPath=path.join(migrationDir,guard.AUDIT_READINESS_MIGRATIONS[0]+'_fixture.sql');
...
for(let slot=0;slot<9;slot++){const paths=canaries.slice();paths[slot]=null;assert.throws(()=>rehearsal('missing_canary_'+slot,paths));check();}
const db=new PGlite();
try{
 ...
 for(const table of ['private.finance_audit_cases_v1','private.finance_audit_case_revisions_v1']){
 ...
 for(const name of ['auth','finalize','utility','ar','profiles','amount','reconcile','taxsource','readiness']){assert.ok(sql.indexOf('$'+name+'_core$')<sql.indexOf('rollback to savepoint'));assert.ok(sql.indexOf('$'+name+'_rollback$')>sql.indexOf('rollback to savepoint'));}
 await db.exec(sql);assert.equal(await fp(),original);check();
 for(let slot=0;slot<9;slot++)for(const failure of ['core','rollback']){const paths=canaries.slice();paths[slot]=canary('failure_'+slot+'_'+failure,30+slot,failure);await assert.rejects(()=>db.exec(rehearsal('rendered_failure_'+slot+'_'+failure,paths)),/intentional/);await db.exec('rollback');assert.equal(await fp(),original);check();}
 for(let i=0;i<postflights.length;i++){
  ...
  for(const mode of ['rehearsal','apply']){await assert.rejects(()=>db.exec(mode==='rehearsal'?rehearsal('bad_post_rehearsal_'+i):apply('bad_post_apply_'+i)),/postflight rejected/);await db.exec('rollback');assert.equal(await fp(),original);assert.equal((await db.query('select count(*)::int as count from supabase_migrations.schema_migrations where version=$1',[guard.AUDIT_READINESS_MIGRATIONS[0]])).rows[0].count,0);check();}
 ...
 const atomic=apply('apply');assert.match(atomic,/pg_advisory_xact_lock[\s\S]+lock table supabase_migrations\.schema_migrations/);assert.equal((atomic.match(/^commit;$/gm)||[]).length,1);for(const name of postflights)assert.ok(atomic.indexOf('-- Reviewed reports postflight: '+name)>atomic.indexOf("'20260911151054',array["));
 await db.query('insert into supabase_migrations.schema_migrations(version) values($1)',['20260911151055']);await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');await db.query('delete from supabase_migrations.schema_migrations where version=$1',['20260911151055']);assert.equal(await fp(),original);check();
 await db.exec(atomic);assert.deepEqual((await db.query('select statements from supabase_migrations.schema_migrations where version=$1',[guard.AUDIT_READINESS_MIGRATIONS[0]])).rows[0].statements,[source]);const applied=await fp();
 await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');assert.equal(await fp(),applied);writeLedger([...prerequisites,...guard.AUDIT_READINESS_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');assert.throws(()=>apply('reapply'),/pending/);check();
 ...
 for(const [domain,markerKey,result] of [['audit_readiness','audit_readiness_canary_result',{canary:'authenticated_audit_readiness_v1',ok:true,rolled_back:true,audit_scope_preserved:true}]]){
 ...
 const actual=path.join(dir,'actual.sql'),args=['prepare-audit-readiness-rehearsal','--migration-dir',path.join(repo,'supabase/migrations'),'--output',actual,'--migration-versions',versions,'--fingerprint',fingerprint,'--receivables-postflight',path.join(repo,'scripts',postflights[5]),'--profiles-postflight',path.join(repo,'scripts',postflights[6])];
 ['authenticated-canary','case-canary','utility-canary','receivables-canary','profiles-canary','amount-search-canary','ar-reconciliation-canary','tax-source-canary','audit-readiness-canary'].forEach((key,i)=>args.push('--'+key,path.join(repo,'scripts',actualCanaries[i])));
 ...
 for(const version of guard.AUDIT_READINESS_MIGRATIONS){const migration=guard.migrationFiles(path.join(repo,'supabase/migrations')).find(name=>name.startsWith(version+'_'));assert.ok(rendered.includes(fs.readFileSync(path.join(repo,'supabase/migrations',migration),'utf8').trimEnd()));}
 ...
 const {createAuditReadinessFixture}=await import('./fixtures/finance_audit_readiness_fixture.cjs');
 const real=new PGlite();
 try{
  await createAuditReadinessFixture(real,{install:false});
  ...
  for(const [i,name] of postflights.entries())fs.writeFileSync(path.join(realDir,name),name==='finance_audit_readiness_postflight.sql'?fs.readFileSync(path.join(repo,'scripts',name),'utf8'):`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);":"select 1;"}\n`);
...
`);
  ...
  guard.prepareAuditBatchRehearsal(path.join(repo,'supabase/migrations'),realRehearsal,versions,fingerprint,inert,realAr,phase,inert,inert,[inert,inert],realProfile,inert,[inert,inert],path.join(repo,'scripts/finance_audit_readiness_canary.sql'));
  ...
  assert.equal((await real.query("select to_regclass('private.finance_audit_cases_v1') as table_name")).rows[0].table_name,null);check();
  const actualPost=path.join(realDir,'finance_audit_readiness_postflight.sql'),actualPostBody=fs.readFileSync(actualPost,'utf8');
  ...
  const realApplied=await realFp();const standalone=await real.exec(fs.readFileSync(path.join(repo,'scripts/finance_audit_readiness_canary.sql'),'utf8'));assert.equal(await realFp(),realApplied,'standalone actual authenticated canary rolls back after installation');
  const realResult=path.join(realDir,'canary.json');fs.writeFileSync(realResult,JSON.stringify(standalone));guard.verifyReportsCanary(realResult,'audit_readiness');check();
 ...
 console.log('PASS audit readiness release: '+checks+' exact phase/prerequisites, nine canaries, eleven pre-COMMIT contracts, full rollback fingerprint and strict CLI checks');
