...
with schema_parts as (
  ...
  from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname in ('public','private','auth')
  union all
  select 'policy:'||schemaname||'.'||tablename||'.'||policyname,concat_ws('|',permissive,roles::text,cmd,qual,with_check)
  from pg_policies where schemaname in ('public','private','auth')
  union all
  ...
  from pg_constraint c join pg_namespace n on n.oid=c.connamespace where n.nspname in ('public','private','auth')
  union all
