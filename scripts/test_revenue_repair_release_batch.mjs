...
try{
 ...
 const preOut=output('prerequisites');guard.prepareRevenueRepairPrerequisiteQuery(path.join(dir,posts[0]),preOut,migrationDir,versions,ledger,baseline);const prior=fs.readFileSync(preOut,'utf8');assert.equal((prior.match(/-- Reviewed reports postflight:/g)||[]).length,23);assert.ok(prior.includes('finance_audit_security_postflight.sql'));assert.ok(!prior.includes('finance_revenue_repair_postflight.sql'));check();
