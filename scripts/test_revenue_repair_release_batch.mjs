...
const repo=fileURLToPath(new URL('..',import.meta.url)),dir=fs.mkdtempSync(path.join(os.tmpdir(),'finance-revenue-repair-release-'));
const migrationDir=path.join(dir,'migrations');fs.mkdirSync(migrationDir);
const phase=guard.RELEASE_PHASE_DATABASE_REVENUE_REPAIR,batch=guard.REVENUE_REPAIR_MIGRATIONS,versions=batch.join(',');
...
const source=`insert into public.revenue_recognition_rules values(1,'reviewed');
insert into public.invoices values(901),(902);
insert into public.ledger_entries values(911),(912),(913),(914);
alter policy attachment_scope_release_policy on storage.objects using ((select public.revenue_repair_release_actor()));
...
const rehearsal=(paths=canaries)=>{const file=output('rehearsal');guard.prepareRevenueRepairRehearsal(migrationDir,file,versions,ledger,fingerprint,...paths,arPost,profilePost,baseline);return fs.readFileSync(file,'utf8');};
...
assert.equal(versions,'20260924074010');check();
...
for(let prefix=0;prefix<=prerequisites.length;prefix++){
 ...
 if(prefix===prerequisites.length){assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'pending');assert.deepEqual(guard.pendingRevenueRepairBatch(migrationDir,versions,ledger,baseline).map(x=>x.version),batch);}
...
writeLedger(prerequisites);assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');check();
...
for(let slot=0;slot<8;slot++){const paths=canaries.slice();paths[slot]=null;assert.throws(()=>rehearsal(paths));check();}
const good=fs.readFileSync(canaries[7],'utf8');
for(const bad of [good.replace('read read only','read'),good.replace('rollback;','commit;'),'\\set ON_ERROR_STOP on\n'+good,good.replace('begin if',"begin perform set_config('request.jwt.claim.sub','fake',true); if"),good.replace('begin if','begin insert into storage.objects values(1); if'),good+'\n-- FINANCE_AUTHENTICATED_CANARY_CORE_BEGIN']){const file=write('unsafe.sql',bad);assert.throws(()=>rehearsal([...canaries.slice(0,7),file]));check();}
const db=new PGlite();
try{
 ...
 await db.exec("create table public.revenue_recognition_rules(id integer,rule text);");
 await db.exec('create table public.file_attachments(id integer);create table storage.objects(id integer);alter table storage.objects enable row level security;create function public.revenue_repair_release_actor() returns boolean language sql stable as $$select true$$;create policy attachment_scope_release_policy on storage.objects for select to authenticated using(public.revenue_repair_release_actor());create function public.revenue_repair_release_probe() returns integer language sql as $$select 0$$;insert into public.application_accounting_lines values(1);');
 ...
 await db.exec("begin;insert into public.revenue_recognition_rules values(99,'changed')");assert.notEqual(await fp(),original,'revenue rules participate in full rollback fingerprint');await db.exec('rollback');assert.equal(await fp(),original);check();
 for(const sql of ['alter policy attachment_scope_release_policy on storage.objects using ((select public.revenue_repair_release_actor()))','grant select on storage.objects to authenticated','insert into storage.objects values(7)','insert into public.file_attachments values(7)']){await db.exec('begin;'+sql);assert.notEqual(await fp(),original,'storage policies, privileges and metadata participate in rollback fingerprint');await db.exec('rollback');assert.equal(await fp(),original);check();}
 ...
 const preOut=output('prerequisites');guard.prepareRevenueRepairPrerequisiteQuery(path.join(dir,posts[0]),preOut,migrationDir,versions,ledger,baseline);const prior=fs.readFileSync(preOut,'utf8');assert.equal((prior.match(/-- Reviewed reports postflight:/g)||[]).length,24);assert.ok(prior.includes('finance_audit_security_postflight.sql'));assert.ok(prior.includes('finance_hr_directory_export_postflight.sql'));assert.ok(!prior.includes('finance_revenue_repair_postflight.sql'));check();
 ...
 for(let slot=0;slot<8;slot++)for(const section of ['core','rollback']){
 ...
 for(const p of [phase,'frontend_compat']){const file=output('recovery');guard.preparePhaseQuery(path.join(dir,posts[0]),file,p,p===phase?versions:'none');const raw=fs.readFileSync(file,'utf8');assert.match(raw,/^begin read only;/);assert.equal((raw.match(/-- Reviewed reports postflight:/g)||[]).length,p===phase?25:24);assert.doesNotMatch(raw,/insert into supabase_migrations|create or replace function/);await db.exec(raw);assert.equal(await fp(),applied);check();}
 const marker='revenue_repair_canary_result',result={canary:'readonly_revenue_repair_v1',ok:true,rolled_back:true,repaired_invoice_count:2,cash_preserved:true},json=write('marker.json','[]');
 for(const payload of [[{[marker]:result}],{rows:[{[marker]:JSON.stringify(result)}]},{results:[{rows:[{[marker]:result}]}]}]){fs.writeFileSync(json,JSON.stringify(payload));assert.equal(guard.verifyReportsCanary(json,'revenue_repair'),true);check();}
 for(const payload of [[],[{[marker]:result},{[marker]:result}],...Object.keys(result).flatMap(k=>[{[marker]:{...result,[k]:false}},{[marker]:Object.fromEntries(Object.entries(result).filter(([key])=>key!==k))}]),{[marker]:{...result,extra:true}}]){fs.writeFileSync(json,JSON.stringify(payload));assert.throws(()=>guard.verifyReportsCanary(json,'revenue_repair'));check();}
 fs.writeFileSync(json,JSON.stringify([{[marker]:result}]));assert.equal(spawnSync(process.execPath,[path.join(repo,'scripts/finance_production_release_guard.js'),'verify-reports-canary','--domain','revenue_repair','--input',json]).status,0);fs.writeFileSync(json,'[]');assert.notEqual(spawnSync(process.execPath,[path.join(repo,'scripts/finance_production_release_guard.js'),'verify-reports-canary','--domain','revenue_repair','--input',json]).status,0);check();
 // Every sealed migration/postflight and all eight marked canary proof blocks survive rendering.
 ...
 console.log(`PASS revenue repair protected release: ${checks} atomic-state and sealed-source checks`);
