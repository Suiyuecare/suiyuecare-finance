...
const {AUDIT_MIGRATIONS,CASE_MIGRATIONS}=require('./finance_production_release_guard');
...
check('route authority and all reviewed production hotfixes are the exact lineage suffix',
  releaseIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 8
    && adoptedRepairIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 7
    && routeHotfixIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 6
    && statusHotfixIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 5
    && finalAccountantHotfixIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 4
    && cashierRepairIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 3
    && cashierSelfDisbursementIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 2
    && humanAccountingIndex === migrations.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 1,
  migrations[migrations.length - 1] || '(none)');
check('audited batch is the exact new ordered suffix',migrations.slice(-AUDIT_MIGRATIONS.length-CASE_MIGRATIONS.length,-CASE_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===AUDIT_MIGRATIONS.join(','));
check('database cases are the exact new ordered suffix',migrations.slice(-CASE_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===CASE_MIGRATIONS.join(','));
for(const version of [...AUDIT_MIGRATIONS,...CASE_MIGRATIONS]){
