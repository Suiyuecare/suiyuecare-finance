...
const {AUDIT_MIGRATIONS,CASE_MIGRATIONS,UTILITY_MIGRATIONS,REPORT_MIGRATIONS}=require('./finance_production_release_guard');
...
check('route authority and all reviewed production hotfixes are the exact lineage suffix',
  releaseIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 8
    && adoptedRepairIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 7
    && routeHotfixIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 6
    && statusHotfixIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 5
    && finalAccountantHotfixIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 4
    && cashierRepairIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 3
    && cashierSelfDisbursementIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 2
    && humanAccountingIndex === migrations.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 1,
  migrations[migrations.length - 1] || '(none)');
check('audited batch is the exact ordered predecessor of cases and utility',migrations.slice(-AUDIT_MIGRATIONS.length-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length,-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===AUDIT_MIGRATIONS.join(','));
check('database cases are the exact ordered predecessor of utility',migrations.slice(-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length,-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===CASE_MIGRATIONS.join(','));
check('utility tax is the exact ordered predecessor of reports',migrations.slice(-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length,-REPORT_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===UTILITY_MIGRATIONS.join(','));
check('financial reports are the exact new ordered suffix',migrations.slice(-REPORT_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===REPORT_MIGRATIONS.join(','));
for(const version of [...AUDIT_MIGRATIONS,...CASE_MIGRATIONS,...UTILITY_MIGRATIONS,...REPORT_MIGRATIONS]){
