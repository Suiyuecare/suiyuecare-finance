...
const {AUDIT_MIGRATIONS,CASE_MIGRATIONS,UTILITY_MIGRATIONS,REPORT_MIGRATIONS,AMOUNT_SEARCH_MIGRATIONS,REPORTING_INTEGRITY_MIGRATIONS,AUDIT_READINESS_MIGRATIONS}=require('./finance_production_release_guard');
...
check('route authority and all reviewed production hotfixes are the exact lineage suffix',
  releaseIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 8
    && adoptedRepairIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 7
    && routeHotfixIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 6
    && statusHotfixIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 5
    && finalAccountantHotfixIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 4
    && cashierRepairIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 3
    && cashierSelfDisbursementIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 2
    && humanAccountingIndex === migrations.length - AUDIT_READINESS_MIGRATIONS.length - REPORTING_INTEGRITY_MIGRATIONS.length - AMOUNT_SEARCH_MIGRATIONS.length - REPORT_MIGRATIONS.length - UTILITY_MIGRATIONS.length - CASE_MIGRATIONS.length - AUDIT_MIGRATIONS.length - 1,
  migrations[migrations.length - 1] || '(none)');
check('audited batch is the exact ordered predecessor of cases and utility',migrations.slice(-AUDIT_MIGRATIONS.length-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===AUDIT_MIGRATIONS.join(','));
check('database cases are the exact ordered predecessor of utility',migrations.slice(-CASE_MIGRATIONS.length-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===CASE_MIGRATIONS.join(','));
check('utility tax is the exact ordered predecessor of reports',migrations.slice(-UTILITY_MIGRATIONS.length-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===UTILITY_MIGRATIONS.join(','));
check('financial reports are the exact predecessor of amount search',migrations.slice(-REPORT_MIGRATIONS.length-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===REPORT_MIGRATIONS.join(','));
check('amount search is the exact predecessor of reporting integrity',migrations.slice(-AMOUNT_SEARCH_MIGRATIONS.length-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===AMOUNT_SEARCH_MIGRATIONS.join(','));
check('reporting integrity is the exact ordered predecessor of audit readiness',migrations.slice(-REPORTING_INTEGRITY_MIGRATIONS.length-AUDIT_READINESS_MIGRATIONS.length,-AUDIT_READINESS_MIGRATIONS.length).map(name=>name.slice(0,14)).join(',')===REPORTING_INTEGRITY_MIGRATIONS.join(','));
check('audit readiness is the exact fixed final suffix',AUDIT_READINESS_MIGRATIONS.join(',')==='20260911151054' && migrations.slice(-AUDIT_READINESS_MIGRATIONS.length).join(',')==='20260911151054_finance_audit_readiness_v1.sql');
for(const version of [...AUDIT_MIGRATIONS,...CASE_MIGRATIONS,...UTILITY_MIGRATIONS,...REPORT_MIGRATIONS,...AMOUNT_SEARCH_MIGRATIONS,...REPORTING_INTEGRITY_MIGRATIONS,...AUDIT_READINESS_MIGRATIONS]){
