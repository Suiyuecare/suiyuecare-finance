...
with schema_parts as (
  select 'namespace:'||n.nspname as key,concat_ws('|',n.nspowner::text,n.nspacl::text) as value from pg_namespace n where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private')
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private') and p.prokind='f'
  union all
  select 'relation:'||c.oid::regclass::text,concat_ws('|',c.relowner::text,c.relkind::text,c.relrowsecurity::text,c.relforcerowsecurity::text,c.relacl::text)
  from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private')
  union all
  select 'policy:'||schemaname||'.'||tablename||'.'||policyname,concat_ws('|',permissive,roles::text,cmd,qual,with_check)
  from pg_policies where schemaname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private')
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private') and a.attnum>0 and not a.attisdropped
  union all
  select 'constraint:'||c.conrelid::regclass::text||'.'||c.conname,pg_get_constraintdef(c.oid,true)
  from pg_constraint c join pg_namespace n on n.oid=c.connamespace where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private')
  union all
  select 'index:'||i.indexrelid::regclass::text,concat_ws('|',pg_get_indexdef(i.indexrelid),pg_get_expr(i.indexprs,i.indrelid),pg_get_expr(i.indpred,i.indrelid),i.indisvalid::text,i.indisready::text)
  from pg_index i join pg_class c on c.oid=i.indrelid join pg_namespace n on n.oid=c.relnamespace
  where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private')
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private','storage','finance_attachment_private') and not t.tgisinternal
), data_parts as (
  ...
  values ('auth','users'),
         ...
         ('private','finance_legacy_attachment_links_v1'),
         ...
         ('public','roles'),
         ('public','employees'),
         ('public','finance_identity_links'),
         ('public','bills'),
         ('private','finance_edoc_member_state_v1'),
         ('private','finance_edoc_company_state_v1'),
         ('private','finance_edoc_sync_outbox_v1'),
         ('private','finance_google_link_mutation_authorizations_v2'),
         ('public','approval_step_actor_snapshots'),
         ('public','invoice_revenue_rule_assignments'),
         ('public','revenue_recognition_rules'),
         ('public','income_document_closure_cases'),
         ('public','approval_admin_director_bottleneck_cases'),
         ...
         ('private','finance_audit_cases_v1'),
         ('private','finance_audit_case_revisions_v1'),
         ('private','finance_payment_concerns_v1'),
         ('private','finance_payment_concern_events_v1'),
         ('public','cash_movement_evidence_links'),
         ('private','finance_history_source_projection_v1'),
         ('private','finance_history_group_projection_v1')
...
-- Audit security includes storage grants, policies and attachment metadata.
-- Complete inherited business fingerprints plus synthetic Auth / HR / identity / eDoc projection side effects.
-- Only digests are returned. No employee records, email, tokens, or source documents are output.

-- Also covers transactional history source/group projection rows, including absent-before-install.

-- Revenue repair additionally fingerprints every revenue recognition rule.
