...
try{
 ...
 writeLedger([...prerequisites,...batch]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/AR read scope migration batch/);assert.throws(()=>rehearsal(),/pending/);assert.throws(()=>apply(),/pending/);check();
 for(const p of [phase]){const file=output('recovery');guard.preparePhaseQuery(path.join(dir,posts[0]),file,p,p===phase?versions:'none');const raw=fs.readFileSync(file,'utf8');assert.match(raw,/^begin read only;/);assert.equal((raw.match(/-- Reviewed reports postflight:/g)||[]).length,20);assert.doesNotMatch(raw,/insert into supabase_migrations|create or replace function/);await db.exec(raw);assert.equal(await fp(),applied);check();}
 ...
 try{
  ...
  for(const p of [phase]){const file=output('true-policy-recovery');guard.preparePhaseQuery(path.join(realDir,posts[0]),file,p,p===phase?versions:'none');await realDb.exec(fs.readFileSync(file,'utf8'));assert.equal(await realFp(),realAfter);check();}
