...
const READ_LATENCY_MIGRATIONS = Object.freeze(['20260913042629']);
const RELEASE_PHASE_DATABASE_READ_LATENCY = 'database_read_latency_20260913';
const READ_LATENCY_POSTFLIGHT_FILES = Object.freeze(['finance_statement_source_postflight.sql']);
...
const APPROVAL_SEARCH_MIGRATIONS = Object.freeze(['20260914091205']);
const APPROVAL_SEARCH_BATCH = Object.freeze([...AUDIT_REMEDIATION_MIGRATIONS,...APPROVAL_SEARCH_MIGRATIONS]);
const RELEASE_PHASE_DATABASE_APPROVAL_SEARCH = 'database_approval_search_20260914';
const APPROVAL_SEARCH_POSTFLIGHT_FILES = Object.freeze(['finance_approval_search_postflight.sql']);
const HISTORY_SUMMARY_MIGRATIONS = Object.freeze(['20260915050313']);
const RELEASE_PHASE_DATABASE_HISTORY_SUMMARY = 'database_history_summary_20260915';
const HISTORY_SUMMARY_POSTFLIGHT_FILES = Object.freeze(['finance_approval_history_summary_postflight.sql']);
const INVOICE_READ_SCOPE_MIGRATIONS = Object.freeze(['20260915080928']);
const RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE = 'database_invoice_read_scope_20260915';
...
const REVENUE_REPAIR_MIGRATIONS = Object.freeze(['20260924074010']);
const RELEASE_PHASE_DATABASE_REVENUE_REPAIR = 'database_revenue_repair_20260924';
...
const REVIEWED_MIGRATION_CATALOG = Object.freeze([
  ...
  ...APPROVAL_SEARCH_MIGRATIONS,
  ...HISTORY_SUMMARY_MIGRATIONS,
...
const RELEASE_PHASES = Object.freeze({
  ...
  [RELEASE_PHASE_DATABASE_APPROVAL_SEARCH]: APPROVAL_SEARCH_BATCH.join(','),
  [RELEASE_PHASE_DATABASE_HISTORY_SUMMARY]: HISTORY_SUMMARY_MIGRATIONS.join(','),
  [RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE]: INVOICE_READ_SCOPE_MIGRATIONS.join(','),
  [RELEASE_PHASE_DATABASE_HR_BRIDGE]: HR_BRIDGE_MIGRATIONS.join(','),
...
const SUPPORTED_GATE_PHASES = Object.freeze([
  ...
  AR_MAPPING_MIGRATIONS,
  AUDIT_REMEDIATION_MIGRATIONS,
  APPROVAL_SEARCH_BATCH,
  HISTORY_SUMMARY_MIGRATIONS,
  INVOICE_READ_SCOPE_MIGRATIONS,
  AR_READ_SCOPE_MIGRATIONS,
  HR_BRIDGE_MIGRATIONS,
  AUDIT_SECURITY_MIGRATIONS,
  REVENUE_REPAIR_MIGRATIONS,
  OPERATIONAL_STABILITY_MIGRATIONS
...
function validateTarget(env, candidate, releasePhase, versionsText, expectedRef) {
  ...
  if (![RELEASE_PHASE_FRONTEND_COMPAT,RELEASE_PHASE_DATABASE_AUDIT_SECURITY,RELEASE_PHASE_DATABASE_REVENUE_REPAIR,RELEASE_PHASE_DATABASE_OPERATIONAL_STABILITY].includes(releasePhase)) fail('legacy database phases are archived for this candidate; use a reviewed current fixed database phase');
...
function classifyLedger(ledgerPath, directory, releasePhase, versionsText, baseline = PRODUCTION_BASELINE_LEDGER) {
  ...
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_REVENUE_REPAIR) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < REVENUE_REPAIR_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('revenue repair phase requires every reviewed prerequisite migration including audit security');
    if (REVENUE_REPAIR_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('revenue repair migration file is missing');
    const installed = REVENUE_REPAIR_MIGRATIONS.filter(version => remote.includes(version));
    if (installed.length && installed.length !== REVENUE_REPAIR_MIGRATIONS.length) fail('revenue repair ledger has a partial atomic migration batch');
    return installed.length ? 'applied' : 'pending';
  }
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_OPERATIONAL_STABILITY) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < OPERATIONAL_STABILITY_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('operational stability phase requires every reviewed prerequisite migration');
  ...
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_AUDIT_SECURITY) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < AUDIT_SECURITY_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('audit security phase requires every reviewed prerequisite migration including the complete HR bridge');
    if (AUDIT_SECURITY_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('audit security migration file is missing');
    const installed = AUDIT_SECURITY_MIGRATIONS.filter(version => remote.includes(version));
    if (installed.length && installed.length !== AUDIT_SECURITY_MIGRATIONS.length) fail('audit security ledger has a partial atomic migration batch');
    return installed.length ? 'applied' : 'pending';
  }
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_HR_BRIDGE) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < HR_BRIDGE_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('HR bridge phase requires every reviewed prerequisite migration');
    if (HR_BRIDGE_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('HR bridge migration file is missing');
    const own = HR_BRIDGE_MIGRATIONS.filter(version => !AR_READ_SCOPE_MIGRATIONS.includes(version));
    const present = own.filter(version => remote.includes(version)).length;
    if (present !== 0 && present !== own.length) fail('HR bridge ledger has a partial atomic migration batch');
    if (present === own.length && AR_READ_SCOPE_MIGRATIONS.some(version => !remote.includes(version))) fail('HR bridge requires its reviewed AR dependency');
    return present === own.length ? 'applied' : 'pending';
  }
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_AR_READ_SCOPE) {
    const hrOwn = HR_BRIDGE_MIGRATIONS.filter(version => !AR_READ_SCOPE_MIGRATIONS.includes(version));
    const hrInstalled = hrOwn.filter(version => remote.includes(version));
    if (hrInstalled.length && (hrInstalled.length !== hrOwn.length || AR_READ_SCOPE_MIGRATIONS.some(version => !remote.includes(version)))) fail('HR bridge ledger has a partial atomic migration batch');
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < AR_READ_SCOPE_MIGRATIONS[0] && !HR_BRIDGE_MIGRATIONS.includes(version));
    if (prerequisites.some(version => !remote.includes(version))) fail('AR read scope phase requires every reviewed prerequisite migration');
    if (AR_READ_SCOPE_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('AR read scope migration file is missing');
    const installed = AR_READ_SCOPE_MIGRATIONS.filter(version => remote.includes(version));
    if (installed.length && installed.length !== AR_READ_SCOPE_MIGRATIONS.length) fail('AR read scope batch is partially installed');
    return installed.length ? 'applied' : 'pending';
  }
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < INVOICE_READ_SCOPE_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('invoice read scope phase requires every reviewed prerequisite migration');
    if (INVOICE_READ_SCOPE_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('invoice read scope migration file is missing');
    const installed = INVOICE_READ_SCOPE_MIGRATIONS.filter(version => remote.includes(version));
    if (installed.length && installed.length !== INVOICE_READ_SCOPE_MIGRATIONS.length) fail('invoice read scope batch is partially installed');
    return installed.length ? 'applied' : 'pending';
  }
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_HISTORY_SUMMARY) {
    const prerequisites = REVIEWED_MIGRATION_CATALOG.filter(version => version < HISTORY_SUMMARY_MIGRATIONS[0]);
    if (prerequisites.some(version => !remote.includes(version))) fail('history summary phase requires every reviewed prerequisite migration');
    if (HISTORY_SUMMARY_MIGRATIONS.some(version => !local.some(name => name.startsWith(version + '_')))) fail('history summary migration file is missing');
    const installed = HISTORY_SUMMARY_MIGRATIONS.filter(version => remote.includes(version));
    if (installed.length && installed.length !== HISTORY_SUMMARY_MIGRATIONS.length) fail('history summary batch is partially installed');
    return installed.length ? 'applied' : 'pending';
  }
  if(plan.releasePhase===RELEASE_PHASE_DATABASE_APPROVAL_SEARCH){
    const prerequisites=REVIEWED_MIGRATION_CATALOG.filter(v=>v<APPROVAL_SEARCH_BATCH[0]);
    if(prerequisites.some(v=>!remote.includes(v)))fail('approval search phase requires every reviewed prerequisite migration');
    if(APPROVAL_SEARCH_BATCH.some(v=>!local.some(name=>name.startsWith(v+'_'))))fail('approval search migration file is missing');
    const pair=AUDIT_REMEDIATION_MIGRATIONS.filter(v=>remote.includes(v));
    if(pair.length&&pair.length!==AUDIT_REMEDIATION_MIGRATIONS.length)fail('approval search refuses a partially installed audit remediation pair');
    const search=remote.includes(APPROVAL_SEARCH_MIGRATIONS[0]);
    if(search&&pair.length!==AUDIT_REMEDIATION_MIGRATIONS.length)fail('approval search migration is installed without its complete audit remediation pair');
    return search?'applied':'pending';
  }
  ...
  if(plan.releasePhase===RELEASE_PHASE_DATABASE_AR_MAPPING){
    ...
    if(AR_MAPPING_MIGRATIONS.some(v=>!local.some(name=>name.startsWith(v+'_'))))fail('AR mapping migration file is missing');
    return remote.includes(AR_MAPPING_MIGRATIONS[0])?'applied':'pending';
  }
  if(plan.releasePhase===RELEASE_PHASE_DATABASE_READ_LATENCY){
    const prerequisites=REVIEWED_MIGRATION_CATALOG.filter(v=>v<READ_LATENCY_MIGRATIONS[0]);
    if(prerequisites.some(v=>!remote.includes(v)))fail('read latency phase requires every reviewed prerequisite migration');
    if(READ_LATENCY_MIGRATIONS.some(v=>!local.some(name=>name.startsWith(v+'_'))))fail('read latency migration file is missing');
    return remote.includes(READ_LATENCY_MIGRATIONS[0])?'applied':'pending';
  }
  ...
  if (plan.releasePhase === RELEASE_PHASE_FRONTEND_COMPAT) {
    ...
    if(APPROVAL_SEARCH_MIGRATIONS.some(version=>!remote.includes(version)))fail('frontend_compat requires the complete approval search migration batch');
    if(HISTORY_SUMMARY_MIGRATIONS.some(version=>!remote.includes(version)))fail('frontend_compat requires the complete history summary migration batch');
    if(INVOICE_READ_SCOPE_MIGRATIONS.some(version=>!remote.includes(version)))fail('frontend_compat requires the complete invoice read scope migration batch');
...
function reportsPostflightChain(postflightPath,profilePostflightPath,includeAmountSearch=false,includeIntegrity=false,includeAuditReadiness=false,includeEmployeeReliability=false,includeHistoryPerformance=false,includeReadLatency=false,includeArMapping=false,includeAuditRemediation=false,includeApprovalSearch=false,includeHistorySummary=false,includeInvoiceReadScope=false,includeArReadScope=false,includeHrBridge=false,includeAuditSecurity=false,includeRevenueRepair=false,includeOperationalStability=false) {
  ...
  return REPORT_POSTFLIGHT_FILES.concat(includeAmountSearch?['finance_amount_search_postflight.sql']:[],includeIntegrity?REPORTING_INTEGRITY_POSTFLIGHT_FILES:[],includeAuditReadiness?AUDIT_READINESS_POSTFLIGHT_FILES:[],includeEmployeeReliability?EMPLOYEE_RELIABILITY_POSTFLIGHT_FILES:[],includeHistoryPerformance?HISTORY_PERFORMANCE_POSTFLIGHT_FILES:[],includeReadLatency?READ_LATENCY_POSTFLIGHT_FILES:[],includeArMapping?AR_MAPPING_POSTFLIGHT_FILES:[],includeAuditRemediation?AUDIT_REMEDIATION_POSTFLIGHT_FILES:[],includeApprovalSearch?APPROVAL_SEARCH_POSTFLIGHT_FILES:[],includeHistorySummary?HISTORY_SUMMARY_POSTFLIGHT_FILES:[],includeInvoiceReadScope?INVOICE_READ_SCOPE_POSTFLIGHT_FILES:[],includeArReadScope?AR_READ_SCOPE_POSTFLIGHT_FILES:[],includeHrBridge?HR_BRIDGE_POSTFLIGHT_FILES:[],includeAuditSecurity?AUDIT_SECURITY_POSTFLIGHT_FILES:[],includeRevenueRepair?REVENUE_REPAIR_POSTFLIGHT_FILES:[],includeOperationalStability?['finance_operational_stability_postflight.sql']:[]).map(name=>{
...
// This phase adds only a SECURITY INVOKER read RPC. Existing writer/RLS contracts
// remain in the complete read-only postflight chain. Its canary creates entirely
// fictional identities and never borrows an employee's authenticated claims.
// Historical canary renderers remain intact for their archived phases.
function prepareReadLatencyRehearsal(directory,outputPath,versionsText,fingerprintPath,canaryPath,postflightPath,profilePostflightPath) {
  const batch=readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_READ_LATENCY);
  const canary=authenticatedCanarySections(canaryPath);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('read latency fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true);
  assertCliAtomicMigration(postflight,'read latency postflight');
  const sources=batch.map(item=>item.source.trimEnd()).join('\n');
  const tag='$finance_read_latency_rollback_assert$';
  if([sources,fingerprint,postflight,canary.core,canary.rollbackCheck].some(value=>value.includes(tag)))fail('read latency rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
create temporary table finance_release_fingerprint_before on commit drop as
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from
     (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'read latency rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function readonlyArMappingCanarySections(canaryPath) {
  if (!canaryPath) fail('AR mapping canary path is required for migration rehearsal');
  const source=fs.readFileSync(canaryPath,'utf8');
  if (!/^--[^\n]*\n(?:--[^\n]*\n)*\s*begin isolation level repeatable read read only;/i.test(source)
      || (source.match(/^\s*rollback\s*;/gim)||[]).length!==1
      || /^\s*(?:commit\s*;|\\)/im.test(source)) fail('AR mapping canary must be a pure SQL read-only repeatable-read rollback transaction');
  ...
  if (/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|call\s+|copy\s+)|request\.jwt|set_config\s*\(/i.test(core+'\n'+rollbackCheck)) fail('AR mapping canary must not write business data or set employee claims');
  return {core,rollbackCheck};
}
function prepareArMappingRehearsal(directory,outputPath,versionsText,fingerprintPath,canaryPath,postflightPath,profilePostflightPath) {
  const batch=readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_AR_MAPPING);
  const canary=readonlyArMappingCanarySections(canaryPath);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('AR mapping fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true);
  assertCliAtomicMigration(postflight,'AR mapping postflight');
  const sources=batch.map(item=>item.source.trimEnd()).join('\n');
  const tag='$finance_ar_mapping_rollback_assert$';
  if([sources,fingerprint,postflight,canary.core,canary.rollbackCheck].some(value=>value.includes(tag)))fail('AR mapping rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
create temporary table finance_release_fingerprint_before on commit drop as
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from
     (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'AR mapping rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
// The operator always selects the complete three-version contract. Only the
// reviewed missing suffix is rendered; a partially installed pair is never repaired.
function pendingApprovalSearchBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH).filter(item=>!remote.includes(item.version));
}
function prepareApprovalSearchPrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingApprovalSearchBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,batch.length===1?RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION:RELEASE_PHASE_DATABASE_AR_MAPPING,batch.length===1?AUDIT_REMEDIATION_MIGRATIONS.join(','):AR_MAPPING_MIGRATIONS.join(','));
}
function prepareApprovalSearchRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingApprovalSearchBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('approval search fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_approval_search_rollback_assert$',ledgerTag='$finance_approval_search_ledger_guard$';
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('approval search rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
...
do ${ledgerTag}
begin
  if (select pg_catalog.array_agg(version order by version) from supabase_migrations.schema_migrations) is distinct from array[${remote.map(sqlLiteral).join(',')}]::text[] then
...
${sources}
${postflight}
${sections.map(x=>x.core).join('\n')}
rollback to savepoint finance_release_migration;
${sections.map(x=>x.rollbackCheck).join('\n')}
create temporary table finance_release_fingerprint_after on commit drop as
${fingerprint}
do ${tag}
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'approval search rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
// This phase starts only from the complete reviewed 2026-09-14 baseline.
function pendingHistorySummaryBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY).filter(item=>!remote.includes(item.version));
}
function prepareHistorySummaryPrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingHistorySummaryBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH,APPROVAL_SEARCH_BATCH.join(','));
}
function prepareHistorySummaryRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingHistorySummaryBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('history summary fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_history_summary_rollback_assert$',ledgerTag='$finance_history_summary_ledger_guard$';
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('history summary rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'history summary rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function pendingInvoiceReadScopeBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE).filter(item=>!remote.includes(item.version));
}
function prepareInvoiceReadScopePrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingInvoiceReadScopeBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,HISTORY_SUMMARY_MIGRATIONS.join(','));
}
function prepareInvoiceReadScopeRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingInvoiceReadScopeBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('invoice read scope fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_invoice_read_scope_rollback_assert$',ledgerTag='$finance_invoice_read_scope_ledger_guard$';
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('invoice read scope rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'invoice read scope rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function pendingArReadScopeBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_AR_READ_SCOPE,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_AR_READ_SCOPE).filter(item=>!remote.includes(item.version));
}
function prepareArReadScopePrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingArReadScopeBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,INVOICE_READ_SCOPE_MIGRATIONS.join(','));
}
function prepareArReadScopeRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingArReadScopeBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('AR read scope fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_ar_read_scope_rollback_assert$',ledgerTag='$finance_ar_read_scope_ledger_guard$';
  const ledgerRows=batch.map(item=>{
    const quote=`$finance_ar_scope_rehearsal_migration_${item.version}$`;
    if(item.source.includes(quote))fail('AR read scope source collides with protected rehearsal ledger tag');
  ...
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('AR read scope rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'AR read scope rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function pendingHrBridgeBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_HR_BRIDGE,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_HR_BRIDGE).filter(item=>!remote.includes(item.version));
}
function prepareHrBridgePrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingHrBridgeBatch(directory,versionsText,ledgerPath,baseline);
  const hasAr=AR_READ_SCOPE_MIGRATIONS.every(version=>readLedgerVersions(ledgerPath).includes(version));
  return preparePhaseQuery(sourcePath,outputPath,hasAr?RELEASE_PHASE_DATABASE_AR_READ_SCOPE:RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,(hasAr?AR_READ_SCOPE_MIGRATIONS:INVOICE_READ_SCOPE_MIGRATIONS).join(','));
}
function prepareHrBridgeRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingHrBridgeBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('HR bridge fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_hr_bridge_rollback_assert$',ledgerTag='$finance_hr_bridge_ledger_guard$';
  const ledgerRows=batch.map(item=>{
    const quote=`$finance_hr_rehearsal_migration_${item.version}$`;
    if(item.source.includes(quote))fail('HR bridge source collides with protected rehearsal ledger tag');
    const name=item.filename.replace(/^\d{14}_/,'').replace(/\.sql$/,'');
    return `insert into supabase_migrations.schema_migrations(version,statements,name,created_by) values (${sqlLiteral(item.version)},array[${quote}${item.source.trimEnd()}${quote}]::text[],${sqlLiteral(name)},'github-actions');`;
  }).join('\n');
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('HR bridge rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'HR bridge rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function pendingAuditSecurityBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_AUDIT_SECURITY,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_AUDIT_SECURITY).filter(item=>!remote.includes(item.version));
}
function prepareAuditSecurityPrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingAuditSecurityBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,RELEASE_PHASE_DATABASE_HR_BRIDGE,HR_BRIDGE_MIGRATIONS.join(','));
}
function prepareAuditSecurityRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,auditSecurityCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingAuditSecurityBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,auditSecurityCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('audit security fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_audit_security_rollback_assert$',ledgerTag='$finance_audit_security_ledger_guard$';
  const ledgerRows=batch.map(item=>{
    const quote=`$finance_audit_security_rehearsal_migration_${item.version}$`;
    if(item.source.includes(quote))fail('audit security source collides with protected rehearsal ledger tag');
    const name=item.filename.replace(/^\d{14}_/,'').replace(/\.sql$/,'');
    return `insert into supabase_migrations.schema_migrations(version,statements,name,created_by) values (${sqlLiteral(item.version)},array[${quote}${item.source.trimEnd()}${quote}]::text[],${sqlLiteral(name)},'github-actions');`;
  }).join('\n');
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('audit security rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'audit security rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function pendingRevenueRepairBatch(directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  verifyLedger('pre',ledgerPath,directory,RELEASE_PHASE_DATABASE_REVENUE_REPAIR,versionsText,baseline);
  const remote=readLedgerVersions(ledgerPath);
  return readAuditBatch(directory,versionsText,RELEASE_PHASE_DATABASE_REVENUE_REPAIR).filter(item=>!remote.includes(item.version));
}
function prepareRevenueRepairPrerequisiteQuery(sourcePath,outputPath,directory,versionsText,ledgerPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  pendingRevenueRepairBatch(directory,versionsText,ledgerPath,baseline);
  return preparePhaseQuery(sourcePath,outputPath,RELEASE_PHASE_DATABASE_AUDIT_SECURITY,AUDIT_SECURITY_MIGRATIONS.join(','));
}
function prepareRevenueRepairRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,auditSecurityCanaryPath,revenueRepairCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
  const batch=pendingRevenueRepairBatch(directory,versionsText,ledgerPath,baseline),remote=readLedgerVersions(ledgerPath);
  const sections=[dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,arReadScopeCanaryPath,auditSecurityCanaryPath,revenueRepairCanaryPath].map(readonlyAuditRemediationCanarySections);
  const fingerprint=stripPsqlDirectives(fs.readFileSync(fingerprintPath,'utf8'),path.basename(fingerprintPath)).trim();
  if(!/^with\b/i.test(fingerprint)||/\b(?:insert\s+into|update\s+\S+\s+set|delete\s+from|alter\s+table|create\s+(?:table|index|schema|function|policy)|drop\s+(?:table|index|schema|function|policy)|truncate\s+|vacuum\b|call\s+|copy\s+)\b/i.test(fingerprint))fail('revenue repair fingerprint must be a pure read-only CTE query');
  const postflight=reportsPostflightChain(postflightPath,profilePostflightPath,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true);
  const sources=batch.map(item=>item.source.trimEnd()).join('\n'),tag='$finance_revenue_repair_rollback_assert$',ledgerTag='$finance_revenue_repair_ledger_guard$';
  const ledgerRows=batch.map(item=>{
    const quote=`$finance_revenue_repair_rehearsal_migration_${item.version}$`;
    if(item.source.includes(quote))fail('revenue repair source collides with protected rehearsal ledger tag');
    const name=item.filename.replace(/^\d{14}_/,'').replace(/\.sql$/,'');
    return `insert into supabase_migrations.schema_migrations(version,statements,name,created_by) values (${sqlLiteral(item.version)},array[${quote}${item.source.trimEnd()}${quote}]::text[],${sqlLiteral(name)},'github-actions');`;
  }).join('\n');
  if([sources,fingerprint,postflight,...sections.flatMap(x=>[x.core,x.rollbackCheck])].some(value=>value.includes(tag)||value.includes(ledgerTag)))fail('revenue repair rehearsal assertion tag collision');
  writeExclusive(outputPath,`begin isolation level repeatable read;
set local lock_timeout = '5s';
set local statement_timeout = '180s';
select pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('finance-production-release-v1',0));
...
begin
  if (select fingerprint from finance_release_fingerprint_before) is distinct from (select fingerprint from finance_release_fingerprint_after) then
    raise exception 'revenue repair rollback rehearsal changed the reviewed database fingerprint';
  end if;
end;
${tag};
rollback;
...
function preparePhaseQuery(sourcePath, outputPath, releasePhase, versionsText) {
  ...
  if(plan.releasePhase===RELEASE_PHASE_DATABASE_REVENUE_REPAIR){
    if(sourceName!=='finance_production_db_postflight.sql')fail('revenue repair requires the reviewed existing-v3 postflight');
    const directory=path.dirname(sourcePath),postflight=reportsPostflightChain(path.join(directory,'finance_canonical_receivables_postflight.sql'),path.join(directory,'finance_reporting_profiles_postflight.sql'),true,true,true,true,true,true,true,true,true,true,true,true,true,true,true);
    writeExclusive(outputPath,`begin read only;\nset local statement_timeout = '60s';\n${postflight}\nrollback;\n`);
    return true;
  }
  if([RELEASE_PHASE_DATABASE_AUDIT_SECURITY,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase)){
    if(sourceName!=='finance_production_db_postflight.sql')fail('audit security compatibility requires the reviewed existing-v3 postflight');
    const directory=path.dirname(sourcePath),postflight=reportsPostflightChain(path.join(directory,'finance_canonical_receivables_postflight.sql'),path.join(directory,'finance_reporting_profiles_postflight.sql'),true,true,true,true,true,true,true,true,true,true,true,true,true,true);
    writeExclusive(outputPath,`begin read only;\nset local statement_timeout = '60s';\n${postflight}\nrollback;\n`);
    return true;
  }
  if([RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_DATABASE_AR_READ_SCOPE,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH,RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase)){
    ...
    const directory=path.dirname(sourcePath),postflight=reportsPostflightChain(path.join(directory,'finance_canonical_receivables_postflight.sql'),path.join(directory,'finance_reporting_profiles_postflight.sql'),true,true,true,true,true,true,true,true,plan.releasePhase!==RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION,[RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_DATABASE_AR_READ_SCOPE,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase),[RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_DATABASE_AR_READ_SCOPE,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase),[RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_DATABASE_AR_READ_SCOPE,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase),[RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase));
    writeExclusive(outputPath,`begin read only;\nset local statement_timeout = '60s';\n${postflight}\nrollback;\n`);
    return true;
  }
...
function verifyReportsCanary(inputPath,domain) {
  const contracts={
    ...
    invoice_read_scope:{marker:'invoice_select_initplan_canary_result',result:{canary:'readonly_invoice_select_initplan_v1',ok:true,rolled_back:true,ordinary_scope_preserved:true}},
    approval_history_summary:{marker:'approval_history_summary_canary_result',result:{canary:'readonly_approval_history_summary_v1',ok:true,rolled_back:true,participant_scope_preserved:true}},
    approval_search:{marker:'approval_search_canary_result',result:{canary:'readonly_approval_search_v1',ok:true,rolled_back:true,participant_scope_preserved:true}},
...
const api = {
  ...
  APPROVAL_SEARCH_MIGRATIONS, APPROVAL_SEARCH_BATCH, RELEASE_PHASE_DATABASE_APPROVAL_SEARCH, APPROVAL_SEARCH_POSTFLIGHT_FILES, pendingApprovalSearchBatch, prepareApprovalSearchPrerequisiteQuery, prepareApprovalSearchRehearsal,
...
if (require.main === module) {
  try {
    ...
    else if (command === 'prepare-hr-bridge-prerequisite-query') prepareHrBridgePrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-hr-bridge-rehearsal') prepareHrBridgeRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('invoice-read-scope-canary'),arg('ar-read-scope-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    else if (command === 'prepare-hr-bridge-apply') prepareAuditBatchApply(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('receivables-postflight'),PRODUCTION_BASELINE_LEDGER,RELEASE_PHASE_DATABASE_HR_BRIDGE,arg('profiles-postflight'));
    else if (command === 'prepare-audit-security-prerequisite-query') prepareAuditSecurityPrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-audit-security-rehearsal') prepareAuditSecurityRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('invoice-read-scope-canary'),arg('ar-read-scope-canary'),arg('audit-security-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    else if (command === 'prepare-audit-security-apply') prepareAuditBatchApply(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('receivables-postflight'),PRODUCTION_BASELINE_LEDGER,RELEASE_PHASE_DATABASE_AUDIT_SECURITY,arg('profiles-postflight'));
    else if (command === 'prepare-revenue-repair-prerequisite-query') prepareRevenueRepairPrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-revenue-repair-rehearsal') prepareRevenueRepairRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('invoice-read-scope-canary'),arg('ar-read-scope-canary'),arg('audit-security-canary'),arg('revenue-repair-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    ...
    else if (command === 'prepare-invoice-read-scope-prerequisite-query') prepareInvoiceReadScopePrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-invoice-read-scope-rehearsal') prepareInvoiceReadScopeRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('invoice-read-scope-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    else if (command === 'prepare-invoice-read-scope-apply') prepareAuditBatchApply(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('receivables-postflight'),PRODUCTION_BASELINE_LEDGER,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,arg('profiles-postflight'));
    else if (command === 'prepare-history-summary-prerequisite-query') prepareHistorySummaryPrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-history-summary-rehearsal') prepareHistorySummaryRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    else if (command === 'prepare-history-summary-apply') prepareAuditBatchApply(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('receivables-postflight'),PRODUCTION_BASELINE_LEDGER,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,arg('profiles-postflight'));
    else if (command === 'prepare-approval-search-prerequisite-query') prepareApprovalSearchPrerequisiteQuery(arg('input'),arg('output'),arg('migration-dir'),arg('migration-versions'),arg('ledger'));
    else if (command === 'prepare-approval-search-rehearsal') prepareApprovalSearchRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
    else if (command === 'prepare-approval-search-apply') prepareAuditBatchApply(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('receivables-postflight'),PRODUCTION_BASELINE_LEDGER,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH,arg('profiles-postflight'));
