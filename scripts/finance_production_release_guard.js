...
const HR_BRIDGE_MIGRATIONS = Object.freeze(['20260922072109','20260922075604']);
...
// explicit frontend_compat/database_invoice_read_scope_20260915 pairs.
...
function validateTarget(env, candidate, releasePhase, versionsText, expectedRef) {
  ...
  if (![RELEASE_PHASE_FRONTEND_COMPAT,RELEASE_PHASE_DATABASE_HR_BRIDGE].includes(releasePhase)) fail('legacy database phases are archived for this candidate; use an exact current database phase');
...
function classifyLedger(ledgerPath, directory, releasePhase, versionsText, baseline = PRODUCTION_BASELINE_LEDGER) {
  ...
  if (plan.releasePhase === RELEASE_PHASE_DATABASE_HR_BRIDGE) {
    ...
    const present = HR_BRIDGE_MIGRATIONS.filter(version => remote.includes(version)).length;
    if (present !== 0 && present !== HR_BRIDGE_MIGRATIONS.length) fail('HR bridge ledger has a partial atomic migration batch');
    return present === HR_BRIDGE_MIGRATIONS.length ? 'applied' : 'pending';
  }
...
function reportsPostflightChain(postflightPath,profilePostflightPath,includeAmountSearch=false,includeIntegrity=false,includeAuditReadiness=false,includeEmployeeReliability=false,includeHistoryPerformance=false,includeReadLatency=false,includeArMapping=false,includeAuditRemediation=false,includeApprovalSearch=false,includeHistorySummary=false,includeInvoiceReadScope=false,includeHrBridge=false) {
...
function prepareHrBridgeRehearsal(directory,outputPath,versionsText,ledgerPath,fingerprintPath,dashboardCanaryPath,googleCanaryPath,searchCanaryPath,summaryCanaryPath,invoiceReadScopeCanaryPath,postflightPath,profilePostflightPath,baseline=PRODUCTION_BASELINE_LEDGER) {
...
function preparePhaseQuery(sourcePath, outputPath, releasePhase, versionsText) {
  ...
  if([RELEASE_PHASE_DATABASE_HR_BRIDGE,RELEASE_PHASE_DATABASE_INVOICE_READ_SCOPE,RELEASE_PHASE_DATABASE_HISTORY_SUMMARY,RELEASE_PHASE_DATABASE_APPROVAL_SEARCH,RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION,RELEASE_PHASE_FRONTEND_COMPAT].includes(plan.releasePhase)){
...
if (require.main === module) {
  try {
    ...
    else if (command === 'prepare-hr-bridge-rehearsal') prepareHrBridgeRehearsal(arg('migration-dir'),arg('output'),arg('migration-versions'),arg('ledger'),arg('fingerprint'),arg('dashboard-scope-canary'),arg('google-projection-canary'),arg('approval-search-canary'),arg('history-summary-canary'),arg('invoice-read-scope-canary'),arg('receivables-postflight'),arg('profiles-postflight'));
