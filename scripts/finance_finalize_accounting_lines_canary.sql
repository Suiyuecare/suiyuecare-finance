-- Authenticated final-accounting canary. Synthetic test rows only; the entire
...
declare
  ...
  v_id constant text:='__finance_finalize_lines_canary_20260908__';
  v_no constant text:='CANARY-FINALIZE-LINES-20260908';
  v_voucher constant text:='V-CANARY-FINALIZE-LINES-20260908';
  v_actor public.finance_users%rowtype;v_applicant public.finance_users%rowtype;
  v_old jsonb;v_new jsonb;v_who jsonb;v_steps jsonb;v_done jsonb;v_entries jsonb;v_result jsonb;v_saved public.expense_requests%rowtype;
...
begin
  if to_regprocedure('private.finance_finalize_accounting_patch_v1(public.expense_requests,jsonb,jsonb,numeric,text)') is null then
    raise exception 'Final accounting canary requires the new atomic persistence helper';end if;
  if exists(select 1 from public.expense_requests where id=v_id or no=v_no)
    ...
    raise exception 'Final accounting canary identifiers already exist; refusing to overwrite';end if;
  ...
  if v_actor.id is null then raise exception 'Final accounting canary has no verified active accountant';end if;
  ...
  if v_applicant.id is null then raise exception 'Final accounting canary has no separate active applicant';end if;
  if not exists(select 1 from public.finance_department_units u join public.finance_department_entity_scopes s
    ...
    raise exception 'Final accounting canary posting scope is unavailable';end if;
  ...
  v_old:=jsonb_build_object('id','line_1','source','detail','description','Rollback-only final accounting canary',
  ...
  if v_old->>'debitAccountName' is null or v_old->>'creditAccountName' is null
    or private.finance_tenant_account_name(v_tenant,'6207') is null then raise exception 'Final accounting canary account catalog is unavailable';end if;
  v_who:=jsonb_build_object('id',v_actor.id,'name',v_actor.name,'role',v_actor.role);
  v_new:=v_old||jsonb_build_object('debitAccount','6207','debitAccountName',private.finance_tenant_account_name(v_tenant,'6207'),
    ...
    'manualOverrideHistory',jsonb_build_array(jsonb_build_object('at',v_at,'actor',v_who,'source','approval_modal_review',
      'changes',jsonb_build_object('debitAccount',jsonb_build_object('before','6299','after','6207')))),
  ...
  values(v_id,v_no,v_tenant,'test','E6','J1101',v_applicant.id,v_applicant.name,v_applicant.email,
    'payment_request','Rollback canary',100,'Rollback-only final accounting canary','pending_voucher',2,1,current_date,
  ...
  v_entries:=jsonb_build_array(jsonb_build_object('t','dr','ac','6207','amt',100),jsonb_build_object('t','cr','ac','1112','amt',100));
  execute 'set local role authenticated';
  if current_user<>'authenticated' or (public.current_finance_user()).id is distinct from v_actor.id
    or public.current_tenant_id() is distinct from v_tenant then raise exception 'Final accounting canary browser identity did not resolve exactly';end if;
  ...
  if v_result->>'ok'<>'true' or v_result->>'idempotent'<>'false' or v_saved.status<>'completed'
    or v_saved.debit_account<>'6207' or v_saved.credit_account<>'1112'
    ...
    or (select count(*) from public.application_accounting_lines where request_id=v_id and data_environment='test' and debit_account='6207' and net_amount=100 and payload=v_new)<>1
    ...
    or not exists(select 1 from public.ledger_entries where tenant_id=v_tenant and data_environment='test' and source_id=v_id and voucher_no=v_voucher and account_code='6207' and debit=100 and credit=0 and voided_at is null)
    ...
    raise exception 'Final accounting canary source, human audit, normalized lines or ledger differ';end if;
...
do $rollback_check$
declare v_id constant text:='__finance_finalize_lines_canary_20260908__';v_no constant text:='CANARY-FINALIZE-LINES-20260908';v_voucher constant text:='V-CANARY-FINALIZE-LINES-20260908';
begin
  if exists(select 1 from public.expense_requests where id=v_id or no=v_no)
    or exists(select 1 from public.application_accounting_lines where request_id=v_id)
    ...
    raise exception 'Final accounting canary rollback left fixture data behind';end if;
end;
$rollback_check$;
-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_END

select jsonb_build_object('canary','authenticated_finalize_accounting_lines','ok',true,'rolled_back',true,'accounting_lines_consistent',true) as finalize_accounting_canary_result;
