...
if(process.env.HR_BRIDGE_BROWSER==='1') {
 ...
 try{
  ...
  // Actual browser engine with manually resolved transports: prove stale
  // success cannot restore salaries after a newer permission denial.
  await role('authenticated',bank);const snapshotRow=(await rpc('finance_hr_snapshot')).obligations.find(x=>x.obligationId===browserObligation);
  const asyncRow={...snapshotRow,status:'awaiting_payment_validation',stageIndex:0,financeVersion:1};
  async function harness(){await page.evaluate(()=>{window.__bridgeCalls=[];const queue=(name,args)=>new Promise(resolve=>window.__bridgeCalls.push({name,args,resolve}));FinanceHrBridge.mount(document.querySelector('#bridge'),{userId:'bank',isCurrent:()=>true,client:{rpc:queue,functions:{invoke:(name,args)=>queue('flush',args)}}});});}
  async function resolveCall(index,data=null,error=null){await page.evaluate(async({index,data,error})=>{window.__bridgeCalls[index].resolve({data,error});await Promise.resolve();await Promise.resolve();},{index,data,error});}
  async function visibleRow(row=asyncRow){await harness();await resolveCall(0,{obligations:[row]});await page.locator('section.card').waitFor();}
  async function denied(){await page.getByText('目前無權讀取此薪資資料，已清除畫面內容。請確認授權後重新讀取。',{exact:true}).waitFor();eq(await page.locator('section.card').count(),0);eq((await page.locator('body').innerText()).includes('1,234.56'),false,'no prior salary remains visible');}
  await harness();await page.getByRole('button',{name:'重新讀取',exact:true}).click();await resolveCall(1,null,{code:'42501',message:'SYNTHETIC_DENIED'});await denied();await resolveCall(0,{obligations:[asyncRow]});eq(await page.locator('section.card').count(),0,'older snapshot cannot restore revoked salary');
  // A late denial also fails closed even if a newer request had succeeded.
  await harness();await page.getByRole('button',{name:'重新讀取',exact:true}).click();await resolveCall(1,{obligations:[asyncRow]});await page.locator('section.card').waitFor();await resolveCall(0,null,{code:'42501',message:'SYNTHETIC_LATE_DENIAL'});await denied();
  // Latest successful empty snapshot also wins against an older success.
  await harness();await page.getByRole('button',{name:'重新讀取',exact:true}).click();await resolveCall(1,{obligations:[]});await resolveCall(0,{obligations:[asyncRow]});eq(await page.locator('section.card').count(),0,'latest empty response wins');
  await visibleRow();await page.locator('input[name="reference"]').fill('SYNTHETIC-ASYNC');await page.locator('input[name="amount"]').fill('1234.56');await page.locator('input[type="file"]').setInputFiles({name:'synthetic.pdf',mimeType:'application/pdf',buffer:Buffer.from('SYNTHETIC')});await page.locator('form button[type="submit"]').click();await page.waitForFunction(()=>window.__bridgeCalls.length===2);await resolveCall(1,null,{code:'42501',message:'SYNTHETIC_DENIED'});await denied();
  // Both voucher reads (the form and the closed voucher) clear all cards.
  for(const status of ['pending_voucher','closed']){await visibleRow({...snapshotRow,status,stageIndex:status==='closed'?5:4});await page.waitForFunction(()=>window.__bridgeCalls.length===2);await resolveCall(1,null,{code:'42501',message:'SYNTHETIC_DENIED'});await denied();}
  // An options denial invalidates another still-pending options response.
  await harness();await resolveCall(0,{obligations:[{...snapshotRow,status:'closed',stageIndex:5},{...snapshotRow,obligationId:'synthetic-second',status:'closed',stageIndex:5}]});await page.waitForFunction(()=>window.__bridgeCalls.length===3);await resolveCall(2,null,{code:'42501',message:'SYNTHETIC_DENIED'});await denied();await resolveCall(1,{voucher:{id:'SYNTHETIC',date:'2026-10-14',description:'PRIVATE',entries:[{t:'dr',ac:'2134',an:'PRIVATE',amt:1234.56}]}});eq(await page.locator('section.card').count(),0,'in-flight voucher details remain invalidated');
  await visibleRow();await page.getByRole('button',{name:'同步進度至人資',exact:true}).click();await resolveCall(1,null,{context:{status:403},message:'SYNTHETIC_EDGE_DENIED'});await denied();
  await visibleRow();await page.getByRole('button',{name:'同步進度至人資',exact:true}).click();await page.getByRole('button',{name:'重新讀取',exact:true}).click();await resolveCall(2,null,{code:'42501',message:'SYNTHETIC_REVOKED'});await denied();await resolveCall(1,{accepted:true,callbackPending:false});eq(await page.getByText('目前無權讀取此薪資資料，已清除畫面內容。請確認授權後重新讀取。',{exact:true}).count(),1,'late flush success cannot overwrite denial');
  // Ambiguous network failures preserve both the draft and idempotency key.
  await visibleRow();await page.locator('input[name="reference"]').fill('SYNTHETIC-RETRY');await page.locator('input[name="amount"]').fill('1234.56');await page.locator('input[type="file"]').setInputFiles({name:'synthetic.pdf',mimeType:'application/pdf',buffer:Buffer.from('SYNTHETIC')});await page.locator('form button[type="submit"]').click();await page.waitForFunction(()=>window.__bridgeCalls.length===2);const uncertainRequest=await page.evaluate(()=>window.__bridgeCalls[1].args.p_request_id);await resolveCall(1,null,{code:'FETCH_ERROR',message:'unknown result'});await page.getByText('目前未能確認完成。請保留佐證並重新讀取狀態；相同內容重試會避免重複登錄。',{exact:true}).waitFor();eq(await page.locator('section.card').count(),1,'network failure retains authorized draft');await page.locator('form button[type="submit"]').click();await page.waitForFunction(()=>window.__bridgeCalls.length===3);eq(await page.evaluate(()=>window.__bridgeCalls[2].args.p_request_id),uncertainRequest,'uncertain retry keeps the original request ID');await resolveCall(2,null,{code:'42501',message:'revoked'});await denied();
