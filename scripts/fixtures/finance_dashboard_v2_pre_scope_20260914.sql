-- Exact production predecessor, prosrc MD5 8de3ece274d222fdf7d7a02949d6475b.
...
begin
  ...
  ledger_base as materialized (
    ...
    where coalesce(l.data_environment, 'production') = v_environment
  ...
  expense_form_posting as (
    ...
    left join ledger_base l
      on l.source_id = r.id or l.source_no = r.no or l.reference_no = r.no
  ...
  invoice_form_posting as (
    ...
    left join ledger_base l on l.source_id = i.id or l.source_no = i.no or l.reference_no = i.no
  ...
  row_counts as (
    select
      (select count(*) from public.ledger_entries where coalesce(data_environment, 'production') = v_environment and voided_at is null and (v_entity is null or entity_id = v_entity)) as ledger_rows,
      (select count(*) from public.expense_requests where coalesce(data_environment, 'production') = v_environment and voided_at is null and (v_entity is null or entity_id = v_entity)) as expense_rows,
      (select count(*) from public.invoices where coalesce(data_environment, 'production') = v_environment and voided_at is null and (v_entity is null or entity_id = v_entity)) as invoice_rows
  )
