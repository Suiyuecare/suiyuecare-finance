...
assert.deepEqual([...workflow.matchAll(/^          - (frontend_compat|database_\S+)$/gm)].map(match=>match[1]), ['frontend_compat','database_ar_read_scope_20260922'], 'Only current frontend and exact AR read scope phase may be dispatched; archived phases remain queryable only');
...
for(const job of [databaseJob,promoteJob]){
 ...
 const preceding=job.slice(0,at).split('\n').filter(line=>line.trimStart().startsWith('if test "$RELEASE_PHASE"')).at(-1);
 for(const selected of [guard.RELEASE_PHASE_DATABASE_AR_READ_SCOPE,'frontend_compat'])assert.equal(spawnSync('sh',['-c',preceding.trim().replace(/; then$/,'')+'; then exit 0; else exit 1; fi'],{env:{...process.env,RELEASE_PHASE:selected}}).status,0);
...
assert.equal(guard.classifyLedger(ledger,migrations,guard.RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION,guard.AUDIT_REMEDIATION_MIGRATIONS.join(','),syntheticBaseline),'applied');
  ...
  fs.appendFileSync(ledger, '20260922072738\n');
  ...
  assert.match(fs.readFileSync(compatGateOutput,'utf8'),/select 10;[\s\S]+select 11;[\s\S]+select 12;[\s\S]+select 13;[\s\S]+select 14;[\s\S]+select 15;[\s\S]+select 16;[\s\S]+select 17;[\s\S]+select 18;[\s\S]+select 19;[\s\S]+select 20;[\s\S]+select 21;/,'Current frontend cannot omit prior and AR scope postflights');
