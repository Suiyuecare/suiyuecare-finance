...
assert.deepEqual(guard.SUPPORTED_GATE_PHASES, [
  [], ['20260826070814'], ['20260826155840'], ['20260827052447'], ['20260902054834'], guard.AUDIT_MIGRATIONS, guard.CASE_MIGRATIONS, guard.UTILITY_MIGRATIONS, guard.REPORT_MIGRATIONS, guard.AMOUNT_SEARCH_MIGRATIONS, guard.REPORTING_INTEGRITY_MIGRATIONS
]);
...
assert.deepEqual(guard.REVIEWED_MIGRATION_CATALOG, [
  ...
  '20260901081807', '20260902054834', ...guard.AUDIT_MIGRATIONS, ...guard.CASE_MIGRATIONS, ...guard.UTILITY_MIGRATIONS, ...guard.REPORT_MIGRATIONS, ...guard.AMOUNT_SEARCH_MIGRATIONS, ...guard.REPORTING_INTEGRITY_MIGRATIONS
]);
...
guard.validateTarget(exactEnvironment,'a'.repeat(40),guard.RELEASE_PHASE_DATABASE_REPORTING_INTEGRITY,guard.REPORTING_INTEGRITY_MIGRATIONS.join(','),catalog.supabaseProjectRef);
...
assert.deepEqual([...workflow.matchAll(/^          - (frontend_compat|database_\S+)$/gm)].map(match=>match[1]), ['frontend_compat','database_reporting_integrity_20260911'], 'Only current frontend and exact reporting integrity phase may be dispatched; archived phases remain queryable only');
...
assert.doesNotMatch(databaseJob.slice(compatAt,integrityPendingAt),/prepare-.*(?:apply|rehearsal)/,'Frontend compatibility never generates mutation SQL');
assert.doesNotMatch(databaseJob.slice(appliedAt),/prepare-(?:reporting-integrity|amount-search|reports|utility|cases|audit)-apply|prepare-apply/,'Applied recovery never reapplies a migration');
