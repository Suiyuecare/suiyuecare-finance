...
assert.deepEqual(guard.RELEASE_PHASES, {
  ...
  database_hr_bridge_20260922: '20260922072109,20260922075604'
});
...
assert.deepEqual([...workflow.matchAll(/^          - (frontend_compat|database_\S+)$/gm)].map(match=>match[1]), ['frontend_compat','database_hr_bridge_20260922'], 'Only current frontend and exact HR bridge phase may be dispatched; archived phases remain queryable only');
...
assert.equal(guard.classifyLedger(ledger,migrations,guard.RELEASE_PHASE_DATABASE_AUDIT_REMEDIATION,guard.AUDIT_REMEDIATION_MIGRATIONS.join(','),syntheticBaseline),'applied');
  ...
  assert.match(fs.readFileSync(compatGateOutput,'utf8'),/select 10;[\s\S]+select 11;[\s\S]+select 12;[\s\S]+select 13;[\s\S]+select 14;[\s\S]+select 15;[\s\S]+select 16;[\s\S]+select 17;[\s\S]+select 18;[\s\S]+select 19;[\s\S]+select 20;/,'Current frontend cannot omit prior and invoice scope postflights');
