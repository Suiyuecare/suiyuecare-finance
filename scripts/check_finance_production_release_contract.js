...
guard.validateTarget(exactEnvironment,'a'.repeat(40),guard.RELEASE_PHASE_DATABASE_READ_LATENCY,guard.READ_LATENCY_MIGRATIONS.join(','),catalog.supabaseProjectRef);
...
assert.deepEqual([...workflow.matchAll(/^          - (frontend_compat|database_\S+)$/gm)].map(match=>match[1]), ['frontend_compat','database_read_latency_20260913'], 'Only current frontend and exact read latency phase may be dispatched; archived phases remain queryable only');
...
assert.match(databaseJob,/name: Prove authenticated submit[^\n]+\n\s+if: inputs\.release_phase != 'database_read_latency_20260913'/);
for(const job of [databaseJob,promoteJob]) {
  const guarded=job.indexOf('if test "$RELEASE_PHASE" != "database_read_latency_20260913"; then');
  assert.ok(guarded>=0);
...
assert.doesNotMatch(databaseJob.slice(compatAt,readLatencyPendingAt),/prepare-.*(?:apply|rehearsal)/,'Frontend compatibility never generates mutation SQL');
...
fs.appendFileSync(ledger,guard.READ_LATENCY_MIGRATIONS.join('\n')+'\n');
  ...
  fs.appendFileSync(ledger, '20260913042630\n');
  ...
  assert.match(fs.readFileSync(compatGateOutput,'utf8'),/select 10;[\s\S]+select 11;[\s\S]+select 12;[\s\S]+select 13;[\s\S]+select 14;/,'Current frontend cannot omit history or read latency postflight');
