...
function receipt(extra={}){
 const calls=[],flags=[],rows=[{id:'one',status:'pending_receipt_review',total:105,rowVersion:2}],ctx={
  S:{user:{id:'ceo',authUserId:'auth-ceo'}},currentTenantId:()=> 'tenant',activeDataEnvironment:()=> 'test',crypto:{randomUUID:()=> 'stable-operation'},
 ...
 ctx.window=ctx;vm.runInNewContext(read('assets/engines/receipt-transactions.js'),ctx);
 return {ctx,calls,rows,flags};
}
function notifications(data={},extra={}){
 const opened=[],readIds=[],alerts=[],queries=[];const ctx={S:{user:{id:'user'},demoLogin:false},INVS:[],REQS:[],BILLS:[],NOTIFS:[{id:'notif',reqId:'one',recordType:'invoices',type:'receipt'}],
  ...
  markNotifRead:async id=>readIds.push(id),alert:text=>alerts.push(text),approvalRowsMarkVerified:()=>{},
  ...
  getSb:()=>({from:table=>{const filters={};return {select(){return this;},eq(key,value){filters[key]=value;return this;},async maybeSingle(){queries.push({table,filters});return {data:data[table]||null};}};}}),...extra};ctx.window=ctx;
 vm.runInNewContext(read('assets/engines/notification-routing.js'),ctx);return {ctx,opened,readIds,alerts,queries};
}
async function rejects(name,fn,code){let e;try{await fn();}catch(error){e=error;}check(name,e&&(!code||e.code===code));}
(async()=>{
 let fixture=receipt(),before=JSON.stringify(fixture.rows),result=await fixture.ctx.financeReceiptAction(fixture.rows,'approve','note',[],{one:2});
 ...
 fixture=receipt();fixture.ctx.getSb=()=>({rpc:async()=>{fixture.ctx.S.user.id='other';return {data:{ok:true,count:1}};}});
 ...
 let n=notifications({invoices:{id:'one'}});check('Invoice receipt notification opens receipt detail',await n.ctx.openFinanceNotification('notif')&&n.opened[0][0]==='receipt'&&n.readIds.length===1&&n.ctx.INVS[0].mapped==='invoice');
 ...
 n=notifications({invoices:{id:'one'}});n.ctx.NOTIFS[0].type='approval';await n.ctx.openFinanceNotification('notif');check('Invoice approval routes to invoice detail',n.opened[0][0]==='invoice');
 n=notifications({bills:{id:'one'}});n.ctx.NOTIFS[0]={id:'notif',reqId:'one',type:'approval'};await n.ctx.openFinanceNotification('notif');check('Legacy untyped bill resolves from exact source',n.opened[0][0]==='bill'&&n.queries.length===3);
 n=notifications({expense_requests:{id:'one'}});n.ctx.NOTIFS[0].recordType='expense_requests';await n.ctx.openFinanceNotification('notif');check('Expense routes to request detail',n.opened[0][0]==='request');
 ...
 n=notifications({invoices:{id:'one'},bills:{id:'one'}});n.ctx.NOTIFS[0]={id:'notif',reqId:'one',type:'approval'};await n.ctx.openFinanceNotification('notif');check('Ambiguous legacy source never guesses or consumes unread',n.readIds.length===0&&n.opened.length===0);
 n=notifications({invoices:{id:'one'}});let identityCalls=0;n.ctx.approvalFastBootstrapIdentity=()=>++identityCalls===1?'old':'new';await n.ctx.openFinanceNotification('notif');check('Account switch during source read does not open old document',n.readIds.length===0&&n.opened.length===0);
