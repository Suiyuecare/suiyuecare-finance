...
const dir=fs.mkdtempSync(path.join(os.tmpdir(),'finance-read-latency-release-'));
const migrationDir=path.join(dir,'migrations');fs.mkdirSync(migrationDir);
const phase=guard.RELEASE_PHASE_DATABASE_READ_LATENCY,versions=guard.READ_LATENCY_MIGRATIONS.join(',');
...
const source='create function public.read_latency_release_probe() returns boolean language sql as $$select true$$;\nupdate public.application_accounting_lines set id=2;';
for(const version of guard.REVIEWED_MIGRATION_CATALOG)fs.writeFileSync(path.join(migrationDir,version+'_fixture.sql'),version===guard.READ_LATENCY_MIGRATIONS[0]?source:'select 1;');
const postflights=guard.REPORT_POSTFLIGHT_FILES.concat('finance_amount_search_postflight.sql',guard.REPORTING_INTEGRITY_POSTFLIGHT_FILES,guard.AUDIT_READINESS_POSTFLIGHT_FILES,guard.EMPLOYEE_RELIABILITY_POSTFLIGHT_FILES,guard.HISTORY_PERFORMANCE_POSTFLIGHT_FILES,guard.READ_LATENCY_POSTFLIGHT_FILES);
const postSources=postflights.map((name,i)=>`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":''}do $post_${i}$ begin if to_regprocedure('public.read_latency_release_probe()') is null or (select id from public.application_accounting_lines)<>2 then raise exception 'postflight must run after migration';end if;end;$post_${i}$;\n`);
...
do $${name}_core$ begin
 ...
 if current_user<>'authenticated' or not public.read_latency_release_probe() then raise exception 'wrong role or missing migration';end if;
...
do $${name}_rollback$ begin
 if exists(select 1 from public.approval_step_actor_snapshots where id<>1) or exists(select 1 from public.invoices) or to_regprocedure('public.read_latency_release_probe()') is not null then raise exception 'canary rollback residue';end if;
...
const rehearsal=(name,paths=canaries)=>{const output=path.join(dir,name+'.sql');guard.prepareReadLatencyRehearsal(migrationDir,output,versions,fingerprint,paths[0],arPost,profilePost);return fs.readFileSync(output,'utf8');};
...
assert.equal(versions,'20260913042629');
assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'pending');
assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/read latency migration batch/);
...
for(const value of ['none',guard.REPORTING_INTEGRITY_MIGRATIONS.join(','),guard.REPORT_MIGRATIONS.join(','),versions+',20260913042630']){assert.throws(()=>guard.releasePlan(phase,value));check();}
writeLedger([...prerequisites,'20260913042630']);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),/unreviewed/);writeLedger(prerequisites);check();
const migrationPath=path.join(migrationDir,guard.READ_LATENCY_MIGRATIONS[0]+'_fixture.sql');
...
try{
 ...
 const atomic=apply('apply');assert.match(atomic,/pg_advisory_xact_lock[\s\S]+lock table supabase_migrations\.schema_migrations/);assert.equal((atomic.match(/^commit;$/gm)||[]).length,1);for(const name of postflights)assert.ok(atomic.indexOf('-- Reviewed reports postflight: '+name)>atomic.indexOf("'20260913042629',array["));
 await db.query('insert into supabase_migrations.schema_migrations(version) values($1)',['20260913042630']);await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');await db.query('delete from supabase_migrations.schema_migrations where version=$1',['20260913042630']);assert.equal(await fp(),original);check();
 await db.exec(atomic);assert.deepEqual((await db.query('select statements from supabase_migrations.schema_migrations where version=$1',[guard.READ_LATENCY_MIGRATIONS[0]])).rows[0].statements,[source]);const applied=await fp();
 await assert.rejects(()=>db.exec(atomic),/ledger changed/);await db.exec('rollback');assert.equal(await fp(),applied);writeLedger([...prerequisites,...guard.READ_LATENCY_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.equal(guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),'compat');assert.throws(()=>apply('reapply'),/pending/);check();
 ...
 for(const [domain,markerKey,result] of [['statement_source','statement_source_canary_result',{canary:'authenticated_statement_source_v1',ok:true,rolled_back:true,source_scope_preserved:true}]]){
 ...
 const actualCanaries=['finance_statement_source_canary.sql'];
 ...
 const actual=path.join(dir,'actual.sql'),args=['prepare-read-latency-rehearsal','--migration-dir',path.join(repo,'supabase/migrations'),'--output',actual,'--migration-versions',versions,'--fingerprint',fingerprint,'--receivables-postflight',path.join(repo,'scripts',postflights[5]),'--profiles-postflight',path.join(repo,'scripts',postflights[6]),'--statement-source-canary',path.join(repo,'scripts/finance_statement_source_canary.sql')];
 ...
 for(const version of guard.READ_LATENCY_MIGRATIONS){const migration=guard.migrationFiles(path.join(repo,'supabase/migrations')).find(name=>name.startsWith(version+'_'));assert.ok(rendered.includes(fs.readFileSync(path.join(repo,'supabase/migrations',migration),'utf8').trimEnd()));}
 ...
 const {createStatementSourceCanaryFixture}=await import('./check_statement_source_canary.cjs');
 const real=new PGlite();
 try{
  await createStatementSourceCanaryFixture(real,{install:false});
  ...
  for(const [i,name] of postflights.entries())fs.writeFileSync(path.join(realDir,name),name==='finance_statement_source_postflight.sql'?fs.readFileSync(path.join(repo,'scripts',name),'utf8'):`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);":"select 1;"}\n`);
  ...
  guard.prepareReadLatencyRehearsal(path.join(repo,'supabase/migrations'),realRehearsal,versions,fingerprint,path.join(repo,'scripts/finance_statement_source_canary.sql'),realAr,realProfile);
  ...
  assert.equal((await real.query("select to_regprocedure('public.finance_statement_source_page_v1(text,text,integer,integer)') as fn")).rows[0].fn,null);check();
  const actualPost=path.join(realDir,'finance_statement_source_postflight.sql'),actualPostBody=fs.readFileSync(actualPost,'utf8');
  ...
  const realApplied=await realFp();const standalone=await real.exec(fs.readFileSync(path.join(repo,'scripts/finance_statement_source_canary.sql'),'utf8'));assert.equal(await realFp(),realApplied,'standalone actual authenticated canary rolls back after installation');
  const realResult=path.join(realDir,'canary.json');fs.writeFileSync(realResult,JSON.stringify(standalone));guard.verifyReportsCanary(realResult,'statement_source');check();
 ...
 console.log('PASS read latency release: '+checks+' exact phase/prerequisites, one synthetic canary, fourteen pre-COMMIT contracts, full rollback fingerprint and strict CLI checks');
