...
with schema_parts as (
  select 'namespace:'||n.nspname as key,concat_ws('|',n.nspowner::text,n.nspacl::text) as value from pg_namespace n where n.nspname='finance_hr_private'
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private') and p.prokind='f'
  union all
  ...
  from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname in ('public','private','auth','finance_hr_private')
  union all
  select 'policy:'||schemaname||'.'||tablename||'.'||policyname,concat_ws('|',permissive,roles::text,cmd,qual,with_check)
  from pg_policies where schemaname in ('public','private','auth','finance_hr_private')
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private') and a.attnum>0 and not a.attisdropped
  union all
  select 'constraint:'||c.conrelid::regclass::text||'.'||c.conname,pg_get_constraintdef(c.oid,true)
  from pg_constraint c join pg_namespace n on n.oid=c.connamespace where n.nspname in ('public','private','auth','finance_hr_private')
  union all
  ...
  where n.nspname in ('public','private','auth','finance_hr_private') and not t.tgisinternal
), data_parts as (
