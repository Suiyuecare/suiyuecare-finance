...
const oldPostflights=guard.REPORT_POSTFLIGHT_FILES.slice(0,-2).map((name,i)=>{
 const sql=`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":''}do $old_postflight_${i}$ begin if (select id from public.application_accounting_lines) is distinct from 3 then raise exception 'old postflight must run after both migrations';end if;end;$old_postflight_${i}$;\n`;
 return{name,path:write(name,sql),sql};
});
write('finance_canonical_receivables_postflight.sql',fs.readFileSync(postflight,'utf8'));
write('finance_reporting_profiles_postflight.sql',fs.readFileSync(profilesPostflight,'utf8'));
write('finance_amount_search_postflight.sql','\\set ON_ERROR_STOP on\nselect 8;\n');
...
try{
 ...
 for(const name of guard.REPORT_POSTFLIGHT_FILES){const at=rehearsal.indexOf('-- Reviewed reports postflight: '+name);assert.ok(at>rehearsal.indexOf(sources[1]),name+' after both migrations');assert.ok(at<rehearsal.indexOf('$auth_core$'),name+' before canaries and rollback');}
 ...
 // Every inherited contract must fail the same transaction BEFORE schema and
 // migration ledger commit, even when both new domain postflights pass.
 for(const old of oldPostflights){
  fs.writeFileSync(old.path,old.sql+"do $fail_old$ begin raise exception 'inherited postflight failed';end;$fail_old$;\n");
  for(const mode of ['rehearsal','apply']){const rendered=mode==='apply'?renderApply('old_failure_apply_'+old.name):renderRehearsal('old_failure_rehearsal_'+old.name);await assert.rejects(()=>db.exec(rendered),/inherited postflight failed/);await db.exec('rollback');assert.equal(await fp(),original);for(const version of guard.REPORT_MIGRATIONS)assert.equal((await db.query('select count(*)::int as count from supabase_migrations.schema_migrations where version=$1',[version])).rows[0].count,0);checks++;}
  fs.writeFileSync(old.path,old.sql);
 }
 for(const old of oldPostflights){fs.unlinkSync(old.path);assert.throws(()=>renderRehearsal('missing_old_rehearsal_'+old.name),/ENOENT/);assert.throws(()=>renderApply('missing_old_apply_'+old.name),/ENOENT/);fs.writeFileSync(old.path,old.sql);checks++;}
 fs.writeFileSync(oldPostflights[0].path,oldPostflights[0].sql+'commit;\n');assert.throws(()=>renderApply('old_postflight_commit'),/must not contain transaction control/);fs.writeFileSync(oldPostflights[0].path,oldPostflights[0].sql);checks++;
 ...
 for(const name of guard.REPORT_POSTFLIGHT_FILES){const at=apply.indexOf('-- Reviewed reports postflight: '+name);assert.ok(at>apply.indexOf(sources[1]),name+' after both migration bodies');assert.ok(at>apply.indexOf("'20260910064325',array["),name+' validates the same transaction as migration ledger');assert.ok(at<apply.lastIndexOf('commit;'),name+' before COMMIT');}
 ...
 const applied=await fp();await assert.rejects(()=>db.exec(apply),/ledger changed/);await db.exec('rollback');assert.equal(await fp(),applied);writeLedger([...prerequisites,...guard.REPORT_MIGRATIONS]);assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'applied');assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/amount search migration/);writeLedger([...prerequisites,...guard.REPORT_MIGRATIONS,...guard.AMOUNT_SEARCH_MIGRATIONS]);assert.throws(()=>guard.classifyLedger(ledger,migrationDir,'frontend_compat','none',baseline),/reporting integrity migration batch/);assert.throws(()=>renderApply('reapply'),/pending/);checks++;
 for(const currentPhase of [phase] /* Current frontend recovery is covered by test_reporting_integrity_release_batch. */){const output=path.join(dir,'applied_recovery_'+currentPhase+'.sql');guard.preparePhaseQuery(oldPostflights[0].path,output,currentPhase,currentPhase===phase?versions:'none');const recovery=fs.readFileSync(output,'utf8');for(const name of guard.REPORT_POSTFLIGHT_FILES)assert.ok(recovery.includes('-- Reviewed reports postflight: '+name));assert.match(recovery,/^begin read only;/);assert.doesNotMatch(recovery,/insert into supabase_migrations|create table public\.(ar|profile)_release_probe/);await db.exec(recovery);assert.equal(await fp(),applied);checks++;}
 ...
 assert.throws(()=>guard.verifyReportsCanary(ledger,'utility'),/unsupported reports canary domain/);
