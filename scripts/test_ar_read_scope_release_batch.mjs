...
import arDomain from './check_ar_verified_accounting_scope.cjs';
...
create table private.ar_read_scope_release_projection(id integer primary key,summary text);
insert into private.ar_read_scope_release_projection values(1,'indexed');
alter table private.ar_read_scope_release_projection enable row level security;
revoke all on table private.ar_read_scope_release_projection from public;
create or replace function public.ar_read_scope_release_probe() returns integer language sql as $$select 1$$;
`;
for(const v of guard.REVIEWED_MIGRATION_CATALOG)write('migrations/'+v+'_fixture.sql',batch.includes(v)?source:'select 1;');
const posts=guard.REPORT_POSTFLIGHT_FILES.concat('finance_amount_search_postflight.sql',guard.REPORTING_INTEGRITY_POSTFLIGHT_FILES,guard.AUDIT_READINESS_POSTFLIGHT_FILES,guard.EMPLOYEE_RELIABILITY_POSTFLIGHT_FILES,guard.HISTORY_PERFORMANCE_POSTFLIGHT_FILES,guard.READ_LATENCY_POSTFLIGHT_FILES,guard.AR_MAPPING_POSTFLIGHT_FILES,guard.AUDIT_REMEDIATION_POSTFLIGHT_FILES,guard.APPROVAL_SEARCH_POSTFLIGHT_FILES,guard.HISTORY_SUMMARY_POSTFLIGHT_FILES,guard.INVOICE_READ_SCOPE_POSTFLIGHT_FILES,guard.AR_READ_SCOPE_POSTFLIGHT_FILES);
assert.equal(posts.length,21);
const postSources=posts.map((name,i)=>`\\set ON_ERROR_STOP on\n${i===0?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":''}do $post_${i}$ begin if public.ar_read_scope_release_probe()<>1 then raise exception 'postflight before migration';end if;end;$post_${i}$;\n`);
...
do $${name}_core$ begin if public.ar_read_scope_release_probe()<>1 or not exists(select 1 from private.ar_read_scope_release_projection where id=1) then raise exception 'canary before migration';end if;end;$${name}_core$;
-- FINANCE_AUTHENTICATED_CANARY_CORE_END
rollback;
-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_BEGIN
do $${name}_rollback$ begin if public.ar_read_scope_release_probe()<>0 or to_regclass('private.ar_read_scope_release_projection') is not null then raise exception 'projection survived rehearsal';end if;end;$${name}_rollback$;
...
const rehearsal=(paths=canaries)=>{const file=output('rehearsal');guard.prepareArReadScopeRehearsal(migrationDir,file,versions,ledger,fingerprint,...paths,arPost,profilePost,baseline);return fs.readFileSync(file,'utf8');};
...
assert.equal(versions,'20260922072737');check();
for(const v of ['none','20260914091205','20260915050313,20260915080928',versions+',20260922072738']){assert.throws(()=>guard.releasePlan(phase,v));check();}
for(let prefix=0;prefix<=prerequisites.length;prefix++){
 writeLedger(prerequisites.slice(0,prefix));
 if(prefix===prerequisites.length){assert.equal(guard.classifyLedger(ledger,migrationDir,phase,versions,baseline),'pending');assert.deepEqual(guard.pendingArReadScopeBatch(migrationDir,versions,ledger,baseline).map(x=>x.version),batch);}
...
for(const rows of [[...prerequisites,'20260922072738'],[...prerequisites,...batch,'20260922072738'],[...prerequisites,batch[0],batch[0]],[...prerequisites].reverse()]){writeLedger(rows);assert.throws(()=>rehearsal());assert.throws(()=>apply());check();}
...
try{
 ...
 const prerequisiteOutput=output('prerequisites');guard.prepareArReadScopePrerequisiteQuery(path.join(dir,posts[0]),prerequisiteOutput,migrationDir,versions,ledger,baseline);const prior=fs.readFileSync(prerequisiteOutput,'utf8');assert.equal((prior.match(/-- Reviewed reports postflight:/g)||[]).length,20);assert.ok(!prior.includes('finance_ar_verified_accounting_scope_postflight.sql'));check();
 ...
 await db.query('insert into supabase_migrations.schema_migrations(version) values($1)',['20260922072738']);
 ...
 guard.prepareArReadScopeRehearsal(actualDir,actualOutput,versions,ledger,path.join(scripts,'finance_approval_history_summary_fingerprint.sql'),...['finance_dashboard_scope_canary.sql','finance_google_projection_canary.sql','finance_approval_search_canary.sql','finance_approval_history_summary_canary.sql','finance_invoice_select_initplan_canary.sql','finance_ar_verified_accounting_scope_canary.sql'].map(name=>path.join(scripts,name)),path.join(scripts,posts[5]),path.join(scripts,posts[6]),baseline);
 ...
 // Execute the actual new reader migration/postflight/canary against deployed
 // authorization function bodies and fictional identities. The twenty other
 ...
 try{
  await arDomain.createFixture(realDb);
  ...
  const invariant="do $real_authority$ begin if to_regprocedure('private.finance_receivables_payload_v1(date,text,text,text,boolean)') is null or to_regprocedure('private.finance_ar_reconciliation_scope_v1(uuid,text,date,text,text)') is null then raise exception 'real source authority missing';end if;end;$real_authority$;";
  for(const name of posts)fs.writeFileSync(path.join(realDir,name),name===guard.AR_READ_SCOPE_POSTFLIGHT_FILES[0]?fs.readFileSync(path.join(scripts,name),'utf8'):'\\set ON_ERROR_STOP on\n'+(name===posts[0]?"select set_config('finance.release_migration_versions', :'migration_versions', true);\n":'')+invariant+'\n');
  const realCanaries=canaries.slice(0,5).map((_,i)=>{const file=path.join(realDir,`inherited-${i}.sql`);fs.writeFileSync(file,`-- Existing domain transport contract, without replacement authority helpers.\nbegin isolation level repeatable read read only;\n-- FINANCE_AUTHENTICATED_CANARY_CORE_BEGIN\n${invariant}\n-- FINANCE_AUTHENTICATED_CANARY_CORE_END\nrollback;\n-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_BEGIN\n${invariant}\n-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_END\n`);return file;}).concat(path.join(scripts,'finance_ar_verified_accounting_scope_canary.sql'));
  const realFp=async()=>(await realDb.query(fpSql)).rows[0].fingerprint,realBefore=await realFp();
  const realRehearsal=output('true-reader-rehearsal');guard.prepareArReadScopeRehearsal(actualDir,realRehearsal,versions,ledger,fingerprint,...realCanaries,path.join(realDir,posts[5]),path.join(realDir,posts[6]),baseline);
  await realDb.exec(fs.readFileSync(realRehearsal,'utf8'));assert.equal(await realFp(),realBefore,'true reader, authorization functions, rows and ledger restore together');check();
  const realApply=output('true-reader-apply');guard.prepareAuditBatchApply(actualDir,realApply,versions,ledger,path.join(realDir,posts[5]),baseline,phase,path.join(realDir,posts[6]));
  const realPost=path.join(realDir,posts.at(-1)),savedPost=fs.readFileSync(realPost,'utf8');fs.appendFileSync(realPost,"do $reject$ begin raise exception 'real reader postflight reject';end;$reject$;\n");const bad=output('true-reader-reject');guard.prepareAuditBatchApply(actualDir,bad,versions,ledger,path.join(realDir,posts[5]),baseline,phase,path.join(realDir,posts[6]));await assert.rejects(()=>realDb.exec(fs.readFileSync(bad,'utf8')),/real reader postflight reject/);await realDb.exec('rollback');assert.equal(await realFp(),realBefore);check();fs.writeFileSync(realPost,savedPost);
