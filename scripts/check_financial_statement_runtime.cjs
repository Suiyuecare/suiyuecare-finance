...
const context={Date,console,Promise,Number,Object,String,Array,Math,window:{FinanceFinancialStatements:engine,FinanceReportingWorkspace:{profileFor:()=>({profile})}},ENTS:[{id:'E1'},{id:'E2'}],LEDGER:[],REQS:[],AUDIT_LOGS:[],remoteLoadPromise:null,STATEMENT_SOURCE_STATE:{ledger:{complete:false},invoices:{complete:false},expense_requests:{complete:false}},LIABILITY_ACCOUNT_ALIASES:{AA1:true,AA2:true},approvalFastBootstrapIdentity:()=>actor,currentTenantId:()=> 'tenant1',activeDataEnvironment:()=>environment,rowDataEnvironment:r=>r.dataEnv||r.data_environment||'production',inActiveDataEnvironment:r=>(r.dataEnv||'production')===environment,rowTenantId:r=>r.tenantId||'tenant1',todayMonth:()=> '2026-09',gD:dc=>({n:dc,eid:'E1'}),num:v=>Number(v)||0,requestCashPostedAt:r=>r.cashPostedAt||'',requestCashAmount:r=>r.amt,fromIsoDate:v=>String(v||'').slice(0,10),todaySlash:()=> '2026/09/10',withOperationTimeout:async p=>p,financeBootstrapRead:async p=>p,isRpcMissing:e=>e&&e.code==='PGRST202',ensureRequestCashPostedFromCeo:()=>{throw Error('Reports must not infer/write cash posting state');},sourceByLedgerRef:ref=>({no:ref}),voucherByLedgerRef:ref=>({no:ref}),financeReportEngine:()=>null};
vm.createContext(context);
const names=['statementDataIdentity','statementDataCompleteness','loadFinanceDocumentSourcePage','loadStatementSourcePages','statementProfileFor','statementMappingsFor','statementScopedLedger','statementProfileEntityIds','statementCashEvents','statementCashFlowOverrides','statementReportModel','statementExportSheets','statementProfitLoss','normDate','entityOfDept','reqEntity','ledgerEntity','inEntity','inPeriod','periodStart','periodEnd','ledgerRowsForEntity','ledgerRows','ledgerRowsToPeriodEnd','ledgerRowsBeforePeriod','ledgerRowsForDepartment','hasLedger','isRevAcct','isExpAcct','isAssetAcct','isLiabAcct','isEquityAcct','isCashAcct','acctLabel','addGroup','rowsFromMap','revenueRows','expenseRows','deptExpenseRows','deptRevenueRows','deptNetRows','financeSummary','balanceRows','requestByRef','companyLoanCashFlowKind','isDuplicateRequestCashLedger','requestStatementCashEvent','requestCashEvents','cashFlow','ledgerTraceRows','statementCashBookBalance','ledgerAmountForKind','ledgerTotalsByKind','ledgerDepartmentComparisonRows','statementCsvCell','statementSheetsCsv'];
...
check('cash book preserves ledger cash while same-number incoming cash cannot hide outgoing request events',()=>{
 ...
 assert.equal(run("isDuplicateRequestCashLedger(LEDGER[2])"),true);assert.equal(run("statementCashBookBalance('E1','2026-09')"),150);assert.equal(run("cashFlow('E1','2026-09').end"),150);assert.equal(run("cashFlow('E1','2026-09').unpostedCashAmount"),-70);context.REQS=[];
});
check('actual cash adapter distinguishes advance principal, settlement and recorded correction bank events',()=>{
 const oldLedger=context.LEDGER;context.expenseCorrectionCashEvents=require('../assets/engines/accounting-corrections.js').cashEvents;
 const req={id:'advance',no:'ADV-SAME-NO',eid:'E1',dc:'D1',type:'advance_request',cashPostedAt:'2026-08-10',amt:100,voucherId:'V-FINAL',formPayload:{advanceDisbursementVoucherId:'V-PRINCIPAL',advanceFinalVoucherId:'V-FINAL',advanceSettlementConfirmed:true,advanceFinalSettledAt:'2026-09-10',advanceSettlementDifference:20,correctionCashEvents:[{correctionId:'C1',bankTransactionId:'B1',date:'2026-09-11',amount:-5},{correctionId:'C1',bankTransactionId:'B2',date:'2026-09-12',amount:3}]}};
 context.REQS=[req,{...req,id:'foreign',eid:'E2'},{...req,id:'test',dataEnv:'test'},{...req,id:'other-tenant',tenantId:'tenant2'}];const before=JSON.stringify(context.REQS);
 const events=plain(run("statementCashEvents('E1','D1')"));assert.equal(events.length,4);const principal=events.find(e=>e.eventType==='disbursement'),settlement=events.find(e=>e.eventType==='settlement');
 assert.equal(principal.eventId,'expense:advance:disbursement');assert.equal(principal.voucherNo,'V-PRINCIPAL');assert.deepEqual(principal.sourceTypes,['advance_disbursement']);assert.equal(principal.amount,-100);
 assert.equal(settlement.eventId,'expense:advance:settlement');assert.equal(settlement.voucherNo,'V-FINAL');assert.deepEqual(settlement.sourceTypes,['expense_request']);assert.equal(settlement.amount,-20);
 assert.deepEqual(events.filter(e=>e.eventType==='correction').map(e=>[e.eventId,e.bankTransactionId,e.amount]),[['expense:advance:correction:C1:B1','B1',-5],['expense:advance:correction:C1:B2','B2',3]]);
 context.LEDGER=[{id:'cash-principal',date:'2026-08-10',eid:'E1',dc:'D1',ac:'1112',cr:100,sourceType:'advance_disbursement',sourceId:'advance',voucherNo:'V-PRINCIPAL'},{id:'temp-principal',date:'2026-08-10',eid:'E1',dc:'D1',ac:'1191',dr:100,sourceType:'advance_disbursement',sourceId:'advance',voucherNo:'V-PRINCIPAL'}];
 const cf=run("cashFlow('E1','2026-09')");assert.equal(cf.start,-100);assert.equal(cf.end,-100);assert.equal(cf.unpostedCashEvents.length,3);assert.equal(cf.unpostedCashAmount,-22);assert.equal(JSON.stringify(context.REQS),before);context.REQS=[];context.LEDGER=oldLedger;delete context.expenseCorrectionCashEvents;
});
check('cash rendering resolves tenant and environment once and never scans all requests per loan override',()=>{
 const originalTenant=context.currentTenantId,originalEnv=context.activeDataEnvironment,originalByRef=context.requestByRef,oldRequests=context.REQS;let tenantReads=0,envReads=0,lookups=0;
 context.currentTenantId=()=>{tenantReads++;return 'tenant1';};context.activeDataEnvironment=()=>{envReads++;return environment;};context.requestByRef=()=>{lookups++;throw Error('Unexpected all-request lookup');};
 context.REQS=Array.from({length:1000},(_,i)=>({id:'loan-'+i,no:'LOAN-'+i,eid:'E1',dc:'D1',cashPostedAt:'2026-09-10',amt:10,formPayload:{shareholderTxn:{type:'intercompany_loan_out',eid:'E1',targetEid:'E2'}}}));
 const before=JSON.stringify(context.REQS),events=run("requestCashEvents('E1','2026-09',false)");assert.equal(events.length,1000);assert.equal(tenantReads,1);assert.equal(envReads,1);tenantReads=0;envReads=0;const overrides=run("statementCashFlowOverrides('all')");assert.equal(Object.keys(overrides).length,2000);assert.equal(overrides['E1|LOAN-0'],'investing');assert.equal(overrides['E2|LOAN-0'],'financing');assert.equal(tenantReads,1);assert.equal(envReads,1);assert.equal(lookups,0);assert.equal(JSON.stringify(context.REQS),before);
 context.currentTenantId=originalTenant;context.activeDataEnvironment=originalEnv;context.requestByRef=originalByRef;context.REQS=oldRequests;
});
check('scoped cash overrides cannot read another tenant with the same request number',()=>{
 const oldRequests=context.REQS;context.REQS=[{id:'foreign',no:'SAME',eid:'E1',tenantId:'tenant2',formPayload:{shareholderTxn:{type:'intercompany_loan_repay',eid:'E1',targetEid:'E2'}}},{id:'own',no:'SAME',eid:'E1',formPayload:{shareholderTxn:{type:'intercompany_loan_out',eid:'E1',targetEid:'E2'}}}];assert.deepEqual(plain(run("statementCashFlowOverrides('all')")),{'E1|SAME':'investing','E2|SAME':'financing'});context.REQS=oldRequests;
});
check('cash adapter cannot invent correction identity when identical records name different bank transactions',()=>{
 const req={id:'expense',eid:'E1',formPayload:{correctionCashEvents:[{correctionId:'C',bankTransactionId:'B1',date:'2026-09-10',amount:-10},{correctionId:'C',bankTransactionId:'B2',date:'2026-09-10',amount:-10}]}};
 context.fixtureRequest=req;context.fixtureEvent={correctionId:'C',date:'2026-09-10',amount:-10};assert.equal(run("requestStatementCashEvent(fixtureRequest,'correction',fixtureEvent).eventId"),'');delete context.fixtureRequest;delete context.fixtureEvent;
});
...
check('raw RPC totals cannot replace mapped financial metrics but canonical AR remains authoritative',()=>{
 context.DASH_REMOTE_AGGREGATES[run('dashboardRemoteKey(scope)')]={version:3,environment:'production',entityId:'all',period:{start:'2026-09-01',end:'2026-09-30',previousStart:'2026-08-01',previousEnd:'2026-08-31',trendStart:'2026-09-01'},rowCounts:{ledger_rows:10,expense_rows:0,invoice_rows:2},summary:{revenue:9999,expense:8888,balanceDiff:0,missingSourceCount:0},previousSummary:{revenue:9999},companies:[{entityId:'E1',revenue:9999}],departments:[{entityId:'E1',departmentCode:'D1',revenue:9999}],trend:[{month:'2026-09',revenue:9999}],receivables:{complete:true,total:321,count:2,overdueTotal:0,over90Total:0,unknownDue:2,buckets:['unknown','not_due','d1','d31','d61','d90'].map(function(key){return{key:key,label:key,count:key==='unknown'?2:0,total:key==='unknown'?321:0};}),items:[{invoiceId:'fictional-i1',entityId:'E1',balance:120,overdueDays:0},{invoiceId:'fictional-i2',entityId:'E2',balance:201,overdueDays:0}]},previousReceivables:{total:300,count:1},reconciliation:{revenue:{officialAmount:9999},expense:{}},generatedAt:'2026-09-10'};
...
check('missing profile blocks financial totals without discarding confirmed canonical AR',()=>{
 ...
 context.bundle=b;run('dashboardRenderKpis(bundle);dashboardRenderVerdict(bundle);dashboardRenderTrend(bundle);dashboardRenderDepartments(bundle)');assert.match(nodes['dash-kpis'].innerHTML,/讀取失敗/);assert.match(nodes['dash-kpis'].innerHTML,/321/);assert.equal(nodes['dash-verdict-title'].textContent,'財務資料讀取失敗');assert.doesNotMatch(nodes['dash-trend-chart'].innerHTML,/沒有正式損益/);assert.match(nodes['dash-dept-body'].innerHTML,/讀取失敗/);records.E2={loaded:true,revision:1};
});
...
(async()=>{
 ...
 client.rpc=async(name,args)=>{assert.equal(name,'finance_statement_source_page_v1');assert.equal(args.p_data_environment,environment);const table=args.p_source,start=args.p_offset,end=start+args.p_limit-1;queries.push({table,start,end,filters:[['tenant_id','tenant1'],['data_environment',environment]]});if(failAfter!=null&&start>=failAfter)return {error:new Error('second page failed')};const rows=data[table].slice(start,end+1).map(row=>({...row,tenant_id:'tenant1',data_environment:environment}));return {data:{ok:true,source:table,tenantId:'tenant1',dataEnvironment:environment,offset:start,limit:args.p_limit,total:data[table].length,rows,hasMore:start+rows.length<data[table].length}};};
