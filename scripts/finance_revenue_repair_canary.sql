-- No employee claims and no business writes.
begin isolation level repeatable read read only;
set local lock_timeout='5s';
set local statement_timeout='15s';
-- FINANCE_AUTHENTICATED_CANARY_CORE_BEGIN
do $no_impersonation$ begin
 if auth.uid() is not null then raise exception 'Revenue repair canary must not impersonate an employee';end if;
end $no_impersonation$;
...
do $revenue_repair_rollback$
begin
 if current_user in ('anon','authenticated','service_role') or auth.uid() is not null then
  raise exception 'Revenue repair proof left a browser role or employee context';end if;
end $revenue_repair_rollback$;
-- FINANCE_AUTHENTICATED_CANARY_ROLLBACK_CHECK_END
select jsonb_build_object('canary','readonly_revenue_repair_v1','ok',true,
 'rolled_back',true,'repaired_invoice_count',2,'cash_preserved',true) as revenue_repair_canary_result;
