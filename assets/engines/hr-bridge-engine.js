(function (global) {
  ...
  async function load(ctx) {
    ...
    try {
      ...
      render(ctx);
    } catch (_) { ctx.rows = []; message(ctx, '交接資料暫時無法讀取。請確認正式服務與薪資授權後重新讀取。'); }
  }
  function render(ctx) {
    if (!valid(ctx)) return;
  ...
  async function showVoucher(ctx, row, card) {
    try { var result = await ctx.client.rpc('finance_hr_voucher_options', { p_obligation_id: row.obligationId }); if (!valid(ctx) || !card.isConnected || result.error || !result.data.voucher) return;
    ...
    } catch (_) { message(ctx, '傳票明細暫時未能讀取，請重新讀取。'); }
  }
  async function voucherForm(ctx, row, card) {
    ...
    try { var response = await ctx.client.rpc('finance_hr_voucher_options', { p_obligation_id: row.obligationId }); if (!valid(ctx) || !card.isConnected) return; if (response.error) throw response.error;
    ...
    } catch (_) { note.textContent = '會計科目暫時無法讀取，請確認權限後重新讀取。'; }
  }
  async function flush(ctx, obligationId) {
    if (!valid(ctx)) return;
    try { var res = await ctx.client.functions.invoke('hr-payroll-intake', { body: { type: 'callback.flush', obligationId: obligationId } }); if (res.error || !res.data || res.data.accepted !== true) throw res.error || new Error('SYNC'); message(ctx, res.data.callbackPending ? 'Finance 紀錄已保留，還有進度等待同步至人資，可稍後重試。' : '進度已同步至人資系統。'); }
    catch (_) { message(ctx, 'Finance 紀錄已保留，人資進度同步尚待完成。可稍後按「同步進度至人資」重試。'); }
  }
  async function submit(ctx, row, action, form) {
    ...
    try {
      ...
      if (!valid(ctx)) return;
      if (result.error) { if (result.error.code) ctx.pending = null; throw result.error; }
      ctx.pending = null; ctx.busy = false; await load(ctx); await flush(ctx, row.obligationId);
    } catch (error) { message(ctx, error && error.message === 'BALANCE' ? '借方與貸方都必須等於核准實發總額，尚未入帳。' : error && /期間已關閉/.test(error.message) ? '此會計期間已關帳，請改用開放期間的日期；本次尚未入帳。' : error && error.message === 'AMOUNT' ? '佐證總額與人資核准實發總額不符，尚未送出。' : error && error.message === 'PENDING' ? '上一筆處理結果仍待確認，請保持原填寫內容重試或重新讀取狀態。' : '目前未能確認完成。請保留佐證並重新讀取狀態；相同內容重試會避免重複登錄。'); }
    finally { ctx.busy = false; if (valid(ctx) && form.isConnected) form.querySelector('button[type="submit"]').disabled = false; }
  }
  global.FinanceHrBridge = { mount: function (element, options) { dispose(); var ctx = Object.assign({ element: element, rows: [], pending: null, alive: true, busy: false }, options); active = ctx; shell(ctx); load(ctx); }, dispose: dispose, escape: escape };
})(window);
