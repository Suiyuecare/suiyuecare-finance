(function(global){
  ...
  async function exportData(kind){var s=copy(scope()),token=identity();await Promise.all([load(s,true),kind==='xlsx'&&typeof global.ensureFinanceDocumentLibrary==='function'?global.ensureFinanceDocumentLibrary('xlsx'):Promise.resolve()]);if(token!==identity()||!active(s))throw new Error('身分或報表範圍已變更，請重新匯出。');var record=recordNow();
    if(kind==='xlsx'){if(!global.XLSX)throw new Error('Excel 匯出元件尚未載入。');var numericControl=record.model.controls.find(function(c){return c.id==='C04';}),financialSheets=numericControl&&numericControl.status==='pass'?global.FinanceFinancialStatements.exportSheets(record.input.statementModel):[{name:'財報待核對',rows:[['公司',s.eid],['期間',s.period],['財報金額尚未匯出','分類帳日期或原始金額仍待確認，請先查看系統核對與來源索引。未知原值不以零替代。']]}],sheets=global.FinanceAuditReadinessEngine.workbookSheets(record.model).concat(financialSheets),wb=global.XLSX.utils.book_new(),used={};sheets.forEach(function(sheet){var name=sheet.name.slice(0,31),base=name,i=2;while(used[name])name=base.slice(0,27)+' '+i++;used[name]=true;var ws=global.XLSX.utils.aoa_to_sheet(sheet.rows);ws['!cols']=[{wch:30},{wch:34},{wch:26},{wch:26},{wch:26},{wch:26}];global.XLSX.utils.book_append_sheet(wb,ws,name);});global.XLSX.writeFile(wb,filename(s,'xlsx'));return;}
